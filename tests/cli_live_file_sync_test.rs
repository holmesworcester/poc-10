...
#[ignore = "poc-8 #61: file sync requires legacy file event/save-file CLI surface (not in substrate)"]
#[test]
fn test_cli_file_slice_projection_is_staggered_during_download() {
    ...
    // poc-8: poc-7's `FILE_SLICE_DATA_BYTES` was renamed to `MAX_SLICE_BYTES`
    // (same value 256 KiB) and lives under `file_slice::codec`.
    let expected_slice_count = (std::fs::metadata(&source_path).unwrap().len() as usize)
        .div_ceil(topo::event_modules::file_slice::codec::MAX_SLICE_BYTES);
