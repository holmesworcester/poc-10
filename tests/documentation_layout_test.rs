...
fn protocol_version_flexibility_design_is_local_and_poc10_specific() {
    ...
    for required in [
        ...
        "The hard version of this design needs a validation frontier DAG",
        "Published capability/version facts act like requests",
        "key-wrap requests and responses",
        "monotonic and hash-linked",
        "F0: active [fallback], deprecated []",
        "C@fallback <----depends-on---- C@v2",
        "To display `C@v3`",
        "downgrading `C@v3` through the declared lens path produces the exact older fact ids",
        "mandatory multi-version publishing becomes consensus-like",
        "permanent fallback format for each durable fact family",
        "always published with the newest/source representation",
        "optional accelerators rather than the thing that makes validation possible",
        "intentionally gives up the stronger invariant that every endpoint receives the best intermediate view",
        "every endpoint can see the permanent fallback",
        "display that newest version against the same fallback",
        ...
        "Handshake capabilities should not decide which durable representations a shared fact needs",
        "publish every durable shared fact in the permanent fallback format and in the newest/source format",
        "Each release manifest records the previous release view versions it can read",
        "intermediate view versions are optional publishing targets rather than required validation anchors",
        "missing optional representations can converge deterministically",
        "signed capability or representation-need facts",
        "accepted optional view versions",
        "Unknown or exotic version claims do not obligate the creator to publish more facts",
        "Sync and connection protocols are the participant-aware special case",
        "The audience is exactly the two peers for the lifetime of the session",
        "Until each side knows the other's authenticated capabilities",
        "session traffic uses the highest mutually available version",
        "If the peer set or capabilities change, start a new session and negotiate again",
        "Invites can make first contact less ambiguous",
        "An invite may name the bootstrap sync-control version",
        "workspace manifest hash",
        "it cannot force a version below the local non-deprecated floor",
        "After admission, the peer publishes signed endpoint and capability facts as shared state",
        "Future peers use those shared facts, not the original invite",
        "Different sync protocol versions negotiate through a fixed bootstrap floor",
        "Start every connection in the lowest non-deprecated sync-control envelope",
        "After authentication, each side sends a transcript-bound capability manifest",
        "supported sync protocol families",
        "minimum accepted version",
        "Select deterministically from the authenticated manifests",
        "choose the highest mutually supported sync version",
        "Bind both complete manifests and the selected version into the session transcript",
        "active network attacker strips newer capabilities",
        "Run the negotiated sync protocol",
        "controls only the session algorithm and envelopes",
        "must not reinterpret their scope or fact-family versions",
        "`sync-v1` might exchange raw fact ids and bytes",
        "`sync-v2` might add representation-set summaries",
        "lowest non-deprecated sync-control envelope",
        "highest mutually supported ephemeral format",
        "fallback facts may live in a parent scope, a private container scope, or per-member scopes",
        "Maximal tactic H: participant-set readiness as optimization, not gate",
        "Core can provide the deterministic version selector",
        "Given participant or device identifiers, their signed capability/version facts",
        "returns the highest common release view version for that participant set",
        "concrete list of missing, stale, or unsupported participants",
        "core only computes the intersection",
        "A sync or connection session is the same calculation with exactly two participants",
        "Event-creating intents should not each hand-roll this decision",
        "asks a compatibility planner for a representation plan",
        "feature id, scope, visibility domain, participant set, accepted manifests, and deprecation policy",
        "mandatory fallback-plus-newest plan for open-ended durable audiences",
        "one highest-common session or closed-set version when fallback is not needed",
        "optional intermediate view versions worth emitting",
        "records the plan hash or manifest ids in the representation-set proof",
        "only an optimization for closed, well-known participant sets",
        "publish only v3",
        "publish fallback plus v3 if the content must remain readable by future devices",
        "permanent fallback plus the richest version the participants can share",
        "For open-ended audiences such as a workspace that may gain new members or devices later",
        "prefer the permanent fallback rule",
        "Maximal tactic I: multi-protocol sessions",
        "Recommended maximal baseline",
        "Every release ships a graph-addressed scope manifest",
        "supported version-graph nodes, release aliases, non-deprecated view versions, bootstrap sync floor, and deprecation policy",
        "Every durable shared fact family declares a permanent fallback format",
        "fallback is the validation anchor",
        "Every durable shared feature declares its visibility domain",
        "typed degradation lens from the richest command or fact to the permanent fallback",
        "Optional lenses to intermediate release view versions are allowed",
        "explicit unsupported-activity fact in an authorized parent scope",
        "one author-certified representation set",
        "For open-ended audiences, publish the permanent fallback plus the richest/source fact",
        "planner may publish only the highest common version",
        "Intermediate release-view siblings are optional",
        "Projectors verify the representation-set proof, scope, audience, version-graph dominance, fallback dependency, and hash links",
        "Readers display the newest verified sibling they understand",
        "Sync negotiates the session protocol through the bootstrap floor and the highest authenticated common sync version",
        "does not define durable compatibility",
        "Deprecation stops new writes of optional intermediate release-view siblings",
        "Permanent fallback and rich/source facts remain until normal retention purges them",
        "Defer true multi-protocol sessions",
        ...
        "participant_ready",
        "internal_only",
        "A feature cannot create a new workspace-visible fact family without either a minimal gate, a legacy fallback mapping, or a maximal permanent-fallback or multi-publish degradation plan",
