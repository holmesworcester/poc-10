...
fn protocol_version_flexibility_research_is_local_and_poc10_specific() {
    ...
    for required in [
        ...
        "Version flexibility should be modeled as feature readiness, not as manual frontend release flags",
        "Developers working on individual features should not need to reason through the whole rollout matrix each time",
        "legacy_fallback",
        "requires_universal_support",
        "internal_only",
        "Feature code should supply the new facts, the old fallback when required, and the upgraded renderer",
        "It should not hand-roll version queries, active-device scans, or deprecation checks",
        "Device capability facts",
        "Workspace readiness",
        "Graceful-degradation contract",
        "Feature embargo",
        "clients older than the supported horizon should get a hard upgrade path",
        "can_create_feature(workspace, feature)",
        "ready_with_legacy_fallback",
        "new interactive content feature can be represented as an old content message plus a new extension fact",
        "prefer embargo over lossy fallback",
        "Adding a feature id requires a manifest entry with one of the compatibility policies above",
        "A feature cannot create a new workspace-visible fact family without either a fallback mapping or a universal-support gate",
        ...
        "Signal is the closest product-policy example",
