...
fn protocol_version_flexibility_design_is_local_and_poc10_specific() {
    ...
    for required in [
        ...
        "**Replay truth.**",
        "Updates wipe materialized state and replay retained facts",
        ...
        "route commands through a protocol-ceiling check",
        "store signed release/canary/time observations as durable local facts",
        "manifest entry naming its first writable ceiling",
    ] {
        assert!(
            normalized.contains(required),
