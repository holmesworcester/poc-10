...
fn poc10_replay_intent_shape_doc_records_current_upgrade_readiness_plan() {
    ...
    for required in [
        ...
        "replay suppresses the emitted intent instead of inserting an intent row",
        "`share_fact_with_sync`",
        "`create_key_wrap`",
        "`unwrap_key_wrap`",
        "`create_connection_response`",
        "`maintain_connections`",
        "The daemon installs it as a live recurring loop after replay",
        ...
        "Add a live recurring `maintain_connections` intent",
        "local `connection_request` rows with a `bootstrap_addr` and no matching `connection_response` row",
        "does not require a separate persisted candidate table",
        "Replay rebuilds request and response rows by replaying retained connection facts",
        ...
        "A later live tick queries the pending bootstrap set",
        ...
        "leaves queued intents, causes network rows, fires recurring schedules, or runs maintenance attempts before the replay barrier",
        ...
        "Bootstrap test: replay rebuilds request/response rows",
