...
fn protocol_version_flexibility_research_is_local_and_poc10_specific() {
    ...
    for required in [
        ...
        "Minimal Design",
        "one canonical production write protocol per feature or scope",
        "Older fact versions still have retained read/replay adapters",
        "old clients may continue writing old facts during their support window",
        ...
        "Keep every non-ephemeral fact replayable into deterministic state",
        "discard derived tables and rebuild them by replaying durable facts through the current registered projectors",
        "TreeKEM",
        "new file encoding",
        ...
        "Database tables, indexes, materialized rows, query caches, and compatibility rows become derived state",
        "The test harness should include replay from fixture fact logs produced by every supported transition version",
        "Replay-on-upgrade needs strict boundaries",
        "Facts are durable protocol truth",
        "replay must not resend network frames",
        "Non-ephemeral local state must be represented as durable local facts, durable queues, or derivable rows until its purge or expiry",
        "local secrets, device keys, pending durable intents, user-visible download progress, correctness-affecting daemon checkpoints",
        "platform capability or permission observations that affect feature eligibility",
        "Replay must include the material needed to make progress",
        "Durable facts, durable local facts, or explicit replay needs must represent every input needed to rebuild deterministic state or schedule required durable work",
        "the relevant local secret material or a durable need for that material must survive until the secret is intentionally purged or retired",
        "Socket state, live sessions, in-flight connection handshakes, and other transport process state are ephemeral",
        "safe to drop and rebuild by reconnecting from durable facts and queues",
        "Ephemeral facts and caches are safe only when losing them cannot remove user-visible work",
        "secret material needed for future decryption",
        "durable obligations",
        "Old-format facts can be replayed by new implementations of old-version adapters",
        "Adapter selection must remain version-addressed",
        "each adapter must preserve the old fact's semantic contract unless an explicit policy/version fact or `effective_from` boundary changes the interpretation",
        "Retention and purge rules vary by workspace",
        "Some fact families, especially auth, may be retained forever",
        "Durable facts are purged only when the data they describe is also purged by policy",
        "Purge, retention, deletion, and disappearance decisions must be expressed as facts before target facts or payloads disappear",
        "replay cannot resurrect data that policy says is gone",
        "does not need Cambria-style lenses at first",
        "keep old versioned codecs and projectors as replay adapters",
        "protocol/v1/auth/key_wrap",
        "project can accumulate supported replay adapters for the support window",
        "workspace retention policies mean some adapters may be long-lived",
        "Auth-like forever facts may require keeping their replay adapters indefinitely",
        "Durable conversion is constrained by signatures",
        "The minimal design keeps old author-signed facts as the provenance source",
        "must not be able to convert an author-signed fact into a new fact that appears to be signed by the original author",
        "That would create impersonation authority",
        "old signed bytes and the matching replay adapter remain the source of truth until the fact is purged under its workspace retention policy",
        "does not rely on signer-authored supersession to retire old content",
        "signers may be offline, gone, or on old devices",
        "Policy facts can publish interpretation, retention, or purge decisions",
        "should not claim to be replacement author signatures",
        "Lens-like conversion rules become useful only as projection rules or as provenance-preserving transformations",
        "should not let a third party rewrite authorship",
        "old adapters are easier and safer than lenses because they preserve the original canonical bytes, hash identity, and author signature",
        "Projectors that normally cause non-ephemeral facts through intents need a replay contract",
        "Replay projectors may declare missing durable work, but they must not execute it directly",
        "recipient X lacks key-wrap coverage for secret Y under format Z",
        "Each produced key-wrap fact has a deterministic coverage key",
        "coverage derivation is only complete if the replay surface also preserves the material needed to satisfy it",
        "A client that still owns the relevant secret must have that secret as a durable local fact",
        "explicit durable need for secret recovery",
        "required key material was treated as disposable cache",
        "leave a durable unsatisfied need rather than fabricating coverage",
        "creation is idempotent and coverage-based, not replay-count-based",
        "Deleting old non-ephemeral facts should also be fact-driven",
        "create a retirement, supersession, or purge-policy fact",
        "Deletion should not be an implicit side effect of replay",
        ...
        "Pure derived database and index changes should prefer replay-on-upgrade",
        "Minimal option D: legacy-visible fallback facts",
        "Recommended minimal path: make read-before-prod-write and replay-derived-state-on-upgrade the default rules",
        "Maximal Design",
