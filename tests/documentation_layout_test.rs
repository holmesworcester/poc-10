...
fn root_readme_describes_context_project_aims() {
    ...
    for required in [
        ...
        "An offer says \"this owner fact's projector published this role/scope/key/value certificate for matching needs.\"",
        "Core only matches role/scope/range overlap",
        "whether that matched offer actually proves what it needs",
...
fn rules_include_projector_style_after_projector_style_doc_was_archived() {
    ...
    for required in [
        ...
        "### Context Proof Style",
        "offer_for(&need)",
        "matched_offers_for(&need)",
        "Legacy payload helpers may remain while old authority paths are migrated",
...
fn fact_authenticator_research_docs_record_authentication_boundary() {
    ...
    for required in [
        ...
        "`encode.rs`",
        "`author.rs`",
        "Context offers are loaded by core through matched needs/offers",
        "producer-owned value bytes through the accepted offer contract",
        "Legacy payload decoding remains migration support",
    ...
    for required in [
        ...
        "requests the version replay rebuild effect and commits the wipe/replay boundary",
...
fn protocol_versioning_docs_separate_release_marker_from_storage_guards() {
    ...
    for required in [
        ...
        "local update fact that runs the version replay rebuild for stale derived state",
        ...
        "emits a local update fact when the database needs a version replay rebuild",
...
fn db_module_docs_explain_identifier_quoting_and_storage_marker_reads() {
    ...
    for required in [
        ...
        "`TableName`, `TypedTableSchema`, `TableInsert`, `TableDeleteWhere`, `ProjectedRowMutation`, `IntentRowMutation`, `RowMutation`, and `Value` are the typed-row mutation vocabulary",
        ...
        "declare a `TypedTableSchema`, build `TableInsert` or `TableDeleteWhere` values from decoded facts, emit `ProjectedRowMutation` or `IntentRowMutation` wrappers",
