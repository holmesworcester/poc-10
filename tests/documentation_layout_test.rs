...
fn protocol_version_flexibility_design_is_local_and_poc10_specific() {
    ...
    for required in [
        ...
        "**Protocol ceiling.**",
        "global provider-scheduled protocol ceiling",
        "Production clients may create, admit, project, and display shared durable protocol",
        "must not register that reader, projector, or command path until the ceiling advances",
        "implementation head",
        "**Release safety.**",
        ...
        "not per peer, group, workspace, or active-client observation",
        ...
        "`user_v1` exposes a name in plaintext",
        "introduce `user_v2` with encrypted profile text",
        "policy fact that hides or quarantines old plaintext names",
        "Intents And Replay",
        "Facts are the compatibility boundary; intents are current-runtime work",
        "old queued intents are not the durable source of correctness",
        "produce ceiling-active durable needs or intent payloads",
        "ensure_key_wrap_coverage_vCurrent(workspace, frontier, recipient, source)",
        "The registered handler fulfills that need idempotently",
        "leaves an unsatisfied durable need rather than fabricating coverage",
        "Connection And Sync",
        "transport format",
        "shared durable protocol",
        "uses the same request version",
        "A v1 sync session may transfer facts produced at the current ceiling",
        "Upgrade Examples",
