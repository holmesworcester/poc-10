...
fn protocol_version_flexibility_design_is_local_and_poc10_specific() {
    ...
    for required in [
        ...
        "Handshake remains useful for ephemeral events and connection-local formats",
        "lowest non-deprecated bootstrap envelope",
        "move up to the highest mutually supported ephemeral format",
        "active network attacker can strip newer capabilities and force a downgrade",
        "Sync has two separate jobs",
        "set-reconciliation over fact ids",
        "a negentropy-style compare finds that a peer is missing fact `X`",
        "The responder looks up `X` in a local index built from fact contents and projection rows",
        "representation_set_id -> sibling fact ids, scope, audience, release view version, and version-graph node",
        "send the missing authorized siblings in the same response",
        "avoids an upgraded client briefly rendering the v1 fallback because the v3 sibling arrives later",
        "the grouping data lives in signed facts and local indexes",
        "reconciling representation-set summaries first",
