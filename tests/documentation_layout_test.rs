...
fn fact_authenticator_research_docs_record_authentication_boundary() {
    ...
    for required in [
        ...
        "`core::pipeline` now exposes first-class route, decode, authenticate, adapt, project, effects, and commit contracts",
        "identity adapt slot",
        ...
        "built two model family shapes",
        "Model fact lessons",
        "`content/message` and `auth/workspace` are the first production staged routes",
        ...
        "all routed fact families are staged, the old composed model is removed",
        "Inventory every remaining legacy family",
        "Do not hand off while any routed family remains on it.",
        "Convert each family to staged route files",
        "Remove the old composed model",
        "The final state has no `FactPipeline::ProjectorComposed`, no `project_authenticated` family delegation, and no `core::projectors` re-export facade.",
    ...
    for required in [
        ...
        "core-managed staged `FactRoute` runner is available for converted facts",
        ...
        "Do not use `core::projectors`, `project_authenticated`, `layout.rs`, `create.rs`, or `rows.rs` as target shapes.",
