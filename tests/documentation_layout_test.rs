...
fn architecture_diagrams_are_github_flowcharts_for_current_context_architecture() {
    ...
    for required in [
        ...
        "becomes durable only after its projector validates request-sent, invite, receipt, and ephemeral-secret context",
        "bootstrap_response opens sealed bytes",
        "create_bootstrap_response handler",
        "response projector",
        "connection_established rows",
...
fn poc10_replay_intent_shape_doc_records_current_upgrade_readiness_plan() {
    ...
    for required in [
        ...
        "`create_bootstrap_response` needs an atomicity fix",
        "It must not send network bytes before the responder ephemeral, response-sent, and `connection_established` facts commit",
