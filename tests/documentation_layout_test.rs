...
fn protocol_version_flexibility_design_is_local_and_poc10_specific() {
    ...
    for required in [
        ...
        "One app provider signs every client release that participates in the privacy contract",
        "arbitrary forks and third-party clients are outside the contract",
        "rapid feature deployment without letting a durable action disappear",
        "Release Registry",
        "The provider publishes a signed releases-registry fact",
        "`release_id`, `manifest_hash`, `status`, `not_before`, and `deprecated_after`",
        "`fallback_fact_version`, `canonical_fact_version`",
        "sync and ephemeral support: bootstrap envelope, protocol versions, and minimum accepted version",
        "`endpoint_id`, user/device id, `release_id`, `manifest_hash`, platform",
        "A capability fact counts only if its release entry is provider-signed and not deprecated",
        "Unknown, forked, or exotic version claims do not create write obligations",
        "`feature_id`, scope, visibility domain",
        "`canonical_to_fallback_lens`",
        "Durable Facts",
        "Every durable shared fact family must define a permanent fallback format",
        "`actor=A, verb=created_calendar_event, title=Team sync, time=Friday 10am`",
        "For every open-ended durable write, the handler emits one representation set",
        "`set_cert`: author signature over set public key",
        "`fallback_fact`: signed by the author or set key",
        "`canonical_fact`: signed by the author or set key; commits to `fallback_id`",
        "Projectors accept the canonical fact only after these checks pass",
        "registry id is accepted, set certificate verifies, fallback fact is admitted",
        "lensing canonical to fallback reproduces `fallback_id`",
        "Readers display the newest verified sibling they understand and suppress lower-fidelity rows only after these checks pass",
        "A private channel fallback cannot go to the whole workspace",
        "Optional Multi-Version Views",
        "Intermediate view facts are UX siblings, not validation anchors",
        "If the author is gone, missing intermediate views are not a correctness failure",
        ...
        "Inputs are the feature manifest, releases registry, endpoint capability facts, and deprecation policy",
        "`DurableOpen`: emit fallback, canonical, and listed optional views",
        "`DurableClosed`: emit fallback plus the highest common view",
        "`Ephemeral`: use bootstrap until capabilities authenticate",
        "`Blocked`: no fallback, unauthorized fallback placement, missing capability",
        "Core can compute the highest common version",
        "core only computes the intersection",
        "Handlers record the planner output hash or manifest ids in `set_cert`",
        "Sync And Ephemeral Traffic",
        "Sync, connection bootstrap, and 1:1 ephemeral traffic have exactly two participants",
        "Start with the invite's bootstrap sync-control version",
        "Authenticate the peer and exchange endpoint capability facts",
        "Verify both releases against the provider registry",
        "Select the highest mutually supported session protocol",
        "Bind both capability facts and the selected protocol into the session transcript",
        ...
        "permanent fallback plus newest canonical representation",
