...
fn protocol_version_flexibility_design_is_local_and_poc10_specific() {
    ...
    for required in [
        ...
        "`create_key_wrap`",
        "`unwrap_key_wrap`",
        "A retained wrap is not enough",
        "purge facts make matching local recipient material unusable",
        "replay must not resurrect the opened secret",
        "`send_facts_on_connection`",
        "`send_network_frame`",
        "`receive_network_frame`",
        "Replay must not fabricate missing key material",
        "Drop queued local sends on upgrade",
        "Connection And Sync",
        "transport format",
