...
fn protocol_version_flexibility_design_is_local_and_poc10_specific() {
    ...
    for required in [
        ...
        "one app provider ships every client that participates in the privacy and compatibility contract",
        "arbitrary forks or third-party clients are outside the contract",
        "a client that enforces weaker local policy, weaker display rules, or weaker retention can become the weak link",
        "rapid feature deployment inside one client family",
        "every non-deprecated client sees either the rich behavior it understands or a durable fallback",
        "Ship read support, fallback display, and test write paths early",
        "provider-signed release registry and a deprecation list",
        "Use permanent fallback facts as validation anchors",
        "Use highest-common version selection for closed participant sets, sync sessions, and ephemeral events",
        "Release Registry",
        "release id, manifest hash, deprecation state",
        "permanent fallback formats, canonical rich formats, optional intermediate view formats",
        "Clients publish signed endpoint capability facts",
        "Only provider-signed release entries create compatibility obligations",
        "Unknown, forked, or exotic version claims do not force writers to publish more facts",
        "Durable Facts",
        "Every durable shared fact family should be introduced with a permanent fallback format",
        "fallback is the validation anchor",
        "explicit unsupported-activity fact in an authorized parent scope",
        "author-certified representation set",
        "the permanent fallback fact",
        "the newest canonical rich fact",
        "optional intermediate view facts for non-deprecated releases",
        "canonical rich fact commits to the fallback fact id",
        "downgrading the rich fact through the declared lens produces the committed fallback id",
        "different versions cannot become semantically different content",
        ...
        "fallback for a private channel, space, or calendar cannot simply publish to the whole workspace",
        "Optional Multi-Version Views",
        "Multi-publishing every non-deprecated release view can improve old-client UX",
        "not the validation backbone",
        "key-wrap requests and responses",
        "hash-linked frontier DAG",
        "C@fallback <----depends-on---- C@v2",
        "too close to a consensus problem",
        "permanent fallback avoids that by making one durable compatibility anchor exist from the beginning",
        "Version Selection",
        "Core can provide a protocol-neutral version selector",
        "provider releases registry, and local deprecation policy",
        "highest common release view version for that participant set",
        "core only computes the intersection",
        "Feature code should use a compatibility planner",
        "for open-ended durable audiences: permanent fallback plus newest canonical",
        "for closed durable participant sets: the highest common version",
        "for ephemeral events and sessions: the bootstrap/fallback format until capabilities are authenticated",
        "Event-creating intents and command handlers should emit the representation set described by the planner",
        "Sync And Ephemeral Traffic",
        "Sync, connection bootstrap, and 1:1 ephemeral traffic are special because the audience is fixed for the session",
        "lowest non-deprecated bootstrap envelope",
        "highest mutually supported session protocol",
        "An invite link may name the bootstrap sync-control version",
        "cannot force a version below the local non-deprecated floor",
        "signed endpoint capability facts become the source of truth",
        "The selected sync version controls only the session algorithm and envelopes",
        "does not reinterpret durable fact semantics",
        "Recommended Baseline",
        "Maintain a provider-signed releases registry and deprecation list",
        "Require provider-signed endpoint capability facts",
        "Publish fallback plus newest canonical facts for open-ended durable audiences",
        "Treat intermediate release-view facts as optional UX siblings",
        "Use a shared compatibility planner",
        "Keep forks, third-party clients, and true multi-protocol sessions outside the baseline privacy and compatibility contract",
        ...
        "participant_ready",
        "internal_only",
        "A feature cannot create a new workspace-visible fact family without either a minimal gate, a legacy fallback mapping, or a one-client-family permanent-fallback degradation plan",
        "The minimal design is the best fit for poc-10 now",
        "The one-client-family compatibility design adds faster feature deployment",
        ...
        "one-client-family design avoids gates only when the planner can publish",
