...
fn topo_sim_bin() -> String {
    // poc-8 has no `topo-sim` binary. All tests in this file are #[ignore]d
    // until/unless a sim binary is reintroduced.
    let _ = bin();
    option_env!("CARGO_BIN_EXE_topo-sim")
        .unwrap_or("/nonexistent/topo-sim")
        .to_string()
}
...
#[test]
#[ignore = "poc-8: requires `topo-sim` binary not present in poc-8"]
