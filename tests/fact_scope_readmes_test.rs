...
fn sync_readme_contains_integrated_projector_owned_negentropy_model() {
    ...
    for required in [
        ...
        "Sync starts once per live connection with an initial seed compare",
        "continues with live-tail sends for newly indexed facts",
        "where catch-up work remains",
        "uses recurring runtime-turn catch-up (`--sync-ms`/`--tick-ms`)",
        ...
        "which live connections may see that owner fact",
        "removes any origin connection recorded by `connection_fact_receipt`",
        "expands the owner through retained `context_have` closure rows",
        "does not create authority and does not bypass projection",
        "without waiting for another compare round",
        "recurring runtime-turn catch-up still repair peers",
        "## Convergence Process",
        "turning projector output into a durable range index",
        "exchanging summaries, exact ids, and the requested fact bytes over live connections",
        ...
        "The retained closure includes each in-range owner",
