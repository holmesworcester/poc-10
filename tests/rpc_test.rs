...
// `DaemonGuard` is re-exported by the local cli_harness module (poc-8 has no
// `topo::testutil` since the substrate dropped that crate-internal helper).
...
fn seed_joining_tenant(db_path: &str) {
    ...
    let (cert, key) = topo::runtime::legacy_identity::generate_self_signed_cert().expect("generate transport key");
...
#[ignore = "poc-8 #61: legacy RPC tests populate retired DB schema (invites_accepted.recorded_by dropped in Stage 3.5 step 5B)"]
#[test]
...
fn rpc_all_methods_serialize() {
    ...
    let methods = vec![
        ...
        // poc-8: `RpcMethod::Files` and `RpcMethod::SaveFile` were dropped
        // — file commands now flow through the projector pipeline rather
        // than dedicated RPC variants.
        RpcMethod::AssertNow {
        ...
        RpcMethod::Reactions,
        RpcMethod::Users { workspace_id: None },
        ...
        RpcMethod::CreateWorkspace {
            ...
            // poc-8: `message_count` / `network_age` simulator fields removed
        },
        ...
        RpcMethod::Peers { workspace_id: None },
        RpcMethod::SubCreate {
            name: "inbox".into(),
        ...
        // poc-8: `RpcMethod::IngestObservability` was removed — observability
        // is now an inbound-event side effect rather than a dedicated RPC.
    ];
...
fn daemon_messages_and_create_workspace_add_new_tenants_with_multi_identity_rows() {
    ...
    let (extra_cert, extra_key) = topo::runtime::legacy_identity::generate_self_signed_cert().unwrap();
    let extra_fp = topo::runtime::legacy_identity::extract_spki_fingerprint(extra_cert.as_ref()).unwrap();
...
fn rpc_peers_returns_local_peer_after_create_workspace() {
    ...
    let resp = topo::rpc::client::rpc_call(
        &socket,
        topo::rpc::protocol::RpcMethod::Peers { workspace_id: None },
    )
    .unwrap();
