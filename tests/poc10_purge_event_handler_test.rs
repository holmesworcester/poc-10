...
#[test]
fn purge_event_missing_target_keeps_intent_queued() {
    let workspace = [1; 32];
...
#[test]
fn purge_event_missing_proof_keeps_intent_queued() {
    let workspace = [3; 32];
...
#[test]
fn purge_event_with_author_proof_purges_target_fact_and_persists() {
    let workspace = [7; 32];
    ...
    assert!(bus
        .intents()
        .iter()
        .any(|intent| intent.kind.as_str() == purge_intent::PURGE_EVENT));
...
#[test]
fn purge_event_invalid_proof_consumes_without_purge() {
    let workspace = [11; 32];
...
#[test]
fn purge_event_handler_does_not_delete_projection_rows() {
    let workspace = [15; 32];
    let signer = [16; 32];
    ...
    bus.submit_fact(deletion);
    bus.submit_intent(intent).expect("submit purge intent");
    bus.dispatch_deferred_intents_with_fact_context(&PurgeEventHandler::new(), 10)
        .expect("purge fact");
...
fn purge_intent(
    ...
    purge_intent::purge_event_intent(PurgeEventIntent {
        workspace_id,
