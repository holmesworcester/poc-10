...
use topo::protocol::auth::key_wrap::fact::{KeyWrapFact, WrappedSecretKind};
...
use topo::protocol::auth::local_recipient_key::layout as local_recipient_layout;
...
use topo::protocol::auth::local_signer_secret::project::LocalSignerSecretProjector;
use topo::protocol::auth::signed_envelope::{create, layout};
use topo::protocol::auth::workspace::scope as workspace_scope;

#[test]
fn signed_envelope_round_trips_and_verifies_key_wrap_payload() {
    let signer_id = [2; 32];
    ...
    let encoded = layout::encode_signed_envelope(&envelope).expect("encode signed envelope");
    let decoded = layout::decode_signed_envelope(&encoded).expect("decode signed envelope");

    assert_eq!(encoded.len(), layout::SIGNED_ENVELOPE_BYTES);
...
#[test]
fn signed_envelope_bytes_are_deterministic_and_define_fact_id() {
    let workspace = [1; 32];
    let signer_id = [2; 32];
    let private_key = [9; 32];
    let payload = key_wrap_layout::encode_key_wrap(&key_wrap()).expect("key wrap payload");
...
#[test]
fn signed_envelope_rejects_tampered_payload_and_signature_context() {
    let signer_id = [2; 32];
    let private_key = [9; 32];
    let payload = key_wrap_layout::encode_key_wrap(&key_wrap()).expect("key wrap payload");
    let mut encoded =
    ...
    let decoded = layout::decode_signed_envelope(&encoded).expect("parse tampered envelope");
    assert!(layout::verify_signed_envelope(&decoded).is_err());

    let payload = key_wrap_layout::encode_key_wrap(&key_wrap()).expect("key wrap payload");
    let mut encoded =
        create::sign_payload_bytes(signer_id, &private_key, payload).expect("signed bytes");
    encoded[1] ^= 1;
    let decoded = layout::decode_signed_envelope(&encoded).expect("parse tampered signer context");
    assert!(layout::verify_signed_envelope(&decoded).is_err());
}

#[test]
fn signed_envelope_uses_fixed_payload_slot_with_canonical_padding() {
    let signer_id = [2; 32];
    let private_key = [9; 32];
    let oversized = vec![
        key_wrap_layout::TYPE_KEY_WRAP;
        topo::protocol::auth::signed_envelope::SIGNED_ENVELOPE_PAYLOAD_BYTES + 1
    ];
    ...
    .expect("signed bytes");
    encoded[layout::SIGNED_ENVELOPE_BYTES - crypto::ED25519_SIGNATURE_BYTES - 1] = 1;
    assert!(layout::decode_signed_envelope(&encoded).is_err());
}
...
fn local_signer_secret_round_trips_and_offers_signing_context() {
    ...
    let bytes = local_signer_layout::encode_fact(&LocalSignerSecretFact {
    ...
    let decoded = local_signer_layout::decode_fact(&fact.bytes).expect("decode signer secret");

    assert_eq!(decoded.workspace_id, workspace);
    assert_eq!(decoded.signer_id, signer_id);
    assert_eq!(decoded.public_key, public_key);
    let output = LocalSignerSecretProjector::new()
...
fn local_signer_secret_must_be_local_scope() {
    ...
    let err = LocalSignerSecretProjector::new()
        .project(&fact, &ProjectionContext::new(Vec::new()))
...
#[test]
fn signed_envelope_rejects_private_payload_tags() {
    let signer_id = [2; 32];
    let private_key = [9; 32];
    let local_signer_payload = local_signer_layout::encode_fact(&LocalSignerSecretFact {
        workspace_id: [1; 32],
        signer_id,
        public_key: crypto::ed25519_public_key(&private_key),
        private_key,
