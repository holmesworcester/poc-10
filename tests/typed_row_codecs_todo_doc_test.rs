...
fn typed_row_codecs_todo_records_row_layout_cleanup_plan() {
    ...
    for required in [
        ...
        "Land the model conversions: `auth/workspace` uses `SchemaSource.row_schemas`",
        "`content/message` removes `rows.rs` by using registry typed-table schemas",
        "Convert the next representative fixed-width opaque row family: `connection/request`",
