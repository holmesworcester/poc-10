...
fn cargo_verus_verifies_production_core_projection_contracts() {
    ...
    assert!(
        combined.contains("verification results:: 97 verified, 0 errors"),
...
fn verus_plan_is_single_core_first_source() {
    ...
    let required = [
        ...
        "shape consumed by `validate_version_replay_rebuild_projection_shape`",
        "proof no longer depends on trusting normalization to preserve offer\nfields",
        "`prepared_projection_commit_fields_accept(context, wakes, effects, owner)`",
        "checks the exact fields",
        "remaining call-order work",
        "verified helpers plus ordinary Rust wrapper calls",
        ...
        "diagnostic rejection branch and full `prepare_projection` call order",
