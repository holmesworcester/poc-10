...
fn core_proofs_make_trust_boundary_explicit() {
    ...
    let required = [
        ...
        "every final, normalized context offer matches some output",
        ...
        "route id/projector info/storage requirement come",
        ...
        "scans the exact prepared fields",
        "all context needs, context offers, time wakes, and purges are\n//!   owned",
        "any version replay rebuild has no standing",
        ...
        "SQL-to-projector replay-mode bridge",
        ...
        "append exactly one need, ownerless\n//!   offer claim, or time wake",
        "The public `ProjectionOutput` context\n//!   builders use these verified production helpers",
        ...
        "leaves the exact `BTreeMap` update theorem for a",
        "The public `ProjectionOutput` work-effect\n//!   builders use these verified production helpers",
        ...
        "`runtime_effects_with_local_intent`",
        "append exactly one item to the named `RuntimeEffects` list",
        "without claiming the map-update relation",
        ...
        "`prepare_projection_output_status` uses this verified helper",
        ...
        "`ProjectionContext::accepted_attested_offer_for`",
        ...
        "Each\n//!   branch uses `dispatch_registered_projector::<P>`",
        "projector call\n//!   and final route stamp come from the same registered projector type",
        "parallel hand-written route fields",
        "The route\n//!   metadata search, field-stamping, selected-stamp helper",
        "dispatch\n//!   finalizer",
        "`prepare_projection` theorem still needs to carry",
        "`PreparedProjection`",
        ...
        "`incoming_fact_with_metadata` now has verified production helpers",
        "map contains exactly the inserted `(fact.id, metadata)` relation",
...
fn verus_plan_is_single_core_first_source() {
    ...
    let required = [
        ...
        "Cargo-verus proves the local replay-mode bridge",
        "`projection_mode_from_replay_flag`",
        "`replay_flag_for_projection_mode` writes exactly `0` for normal and `1` for",
        "The basic builder shape is now uniform",
        "`projection_output_with_need`, `projection_output_with_offer`, and",
        "`projection_output_with_self_purge`, `projection_output_with_fact`",
        "`projection_output_with_incoming_fact_metadata`",
        "`runtime_effects_with_fact`, `runtime_effects_with_priority_fact`",
        "`runtime_effects_with_incoming_fact_metadata`",
        "For metadata builders, the proved claim is",
        "the incoming-fact vector append",
        "a precise `BTreeMap` update theorem still",
        ...
        "shape consumed by the unified prepare-stage guard",
        "proof\nno longer depends on trusting normalization to preserve offer fields",
        "`prepared_projection_commit_fields_accept(context, wakes, effects, owner)`",
        ...
        "plain\n`ProjectionOutput` plus route-stamped `ProjectionRouteEvidence`",
        "Terminology in this stage:",
        "`ProjectionDispatcher`: the production entry point used by `project_fact`",
        "`FactRoute`: one registry row from a fact tag to the route id",
        "`FactRouteId`: stable proof identity for the route",
        ...
        "`ProjectionDispatcher::dispatch_projection`",
        "direct `match effective_tag` branch",
        "`dispatch_registered_projector::<P>`",
        "same projector type\nprovides the branch tag, finalizer stamp",
        ...
        "no longer the success decision\nused by the unified prepare-stage guard",
        "exported\ntheorem correspondence to the full `prepare_projection` sequence",
        ...
        "verified prepare-status result",
