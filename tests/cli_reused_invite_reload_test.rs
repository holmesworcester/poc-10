// Gated off after the legacy `workspace::commands::accept_invite` was
// retired in the recorded_by sweep; the substrate's substitute
// (`Command::AcceptInvite { invite_blob }`) only takes a binary blob,
// not a `topo://invite` URL, so this test would need a substantial
// rewrite before it runs again.
#![cfg(any())]
...
#[ignore = "poc-8 #61: invite reload requires legacy identity-chain"]
#[test]
