...
#[test]
fn well_formed_bundle_frame_without_incoming_metadata_is_rejected() {
    on_big_stack(|| {
        ...
        let err = ConnectionFrameBundleProjector::new()
            .project(&input_fact, &ProjectionContext::default())
            .expect_err("missing incoming metadata should reject direct projection");
        assert!(err.contains("missing incoming origin metadata"));
    });
}
