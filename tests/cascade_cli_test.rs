mod cli_harness;

use std::time::Instant;

use cli_harness::*;

#[test]
fn cascade_cli_replays_fact_with_deps_out_of_order_and_unblocks_10k() {
    let tmp = tempfile::tempdir().unwrap();
    let db = temp_db(&tmp, "cascade.db");

    let staged = assert_success(topo(&["--db", &db, "generate-deps", "10000", "10"]));
    assert_eq!(line_value(&staged, "staged_facts"), "10000");
    assert_eq!(line_value(&staged, "deps_per_fact"), "10");
    assert_eq!(line_value(&staged, "dep_edges"), "99945");
    let status = assert_success(topo(&["--db", &db, "count"]));
    assert_eq!(
        line_value(&status, "facts"),
        "0",
        "staged fixtures must be local-only"
    );

    let started = Instant::now();
    let replayed = assert_success(topo(&["--db", &db, "replay-deps-reverse"]));
    let elapsed = started.elapsed();

    assert_eq!(line_value(&replayed, "replayed_facts"), "10000");
    assert_eq!(line_value(&replayed, "applied_facts"), "10000");

    let status = assert_success(topo(&["--db", &db, "count"]));
    assert_eq!(line_value(&status, "facts"), "10000");
    assert_eq!(line_value(&status, "applied_facts"), "10000");

    let seconds = elapsed.as_secs_f64().max(0.001);
    let rate = 10_000f64 / seconds;
    eprintln!("black_box_cascade_10k facts_per_s={rate:.0}");
    assert!(rate.is_finite() && rate > 0.0);
}

#[test]
#[ignore = "large cascade perf fixture is kept manual; 10k coverage exercises the target runtime path"]
fn cascade_cli_replays_fact_with_deps_out_of_order_and_unblocks_50k() {
    let tmp = tempfile::tempdir().unwrap();
    let db = temp_db(&tmp, "cascade-50k.db");

    let staged = assert_success(topo(&["--db", &db, "generate-deps", "50000", "10"]));
    assert_eq!(line_value(&staged, "staged_facts"), "50000");

    let started = Instant::now();
    let replayed = assert_success(topo(&["--db", &db, "replay-deps-reverse"]));
    let elapsed = started.elapsed();

    assert_eq!(line_value(&replayed, "replayed_facts"), "50000");
    assert_eq!(line_value(&replayed, "applied_facts"), "50000");

    let seconds = elapsed.as_secs_f64().max(0.001);
    let rate = 50_000f64 / seconds;
    eprintln!("black_box_cascade_50k facts_per_s={rate:.0}");
}
