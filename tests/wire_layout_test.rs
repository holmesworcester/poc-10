...
fn connection_frame_ciphertext_slot_layout_includes_aead_tag_capacity() {
    ...
    assert!(CONNECTION_FRAME_BUNDLE_PLAINTEXT_BYTES > CONNECTION_FRAME_SMALL_PLAINTEXT_BYTES);
    assert!(CONNECTION_FRAME_FILE_SLICE_PLAINTEXT_BYTES > CONNECTION_FRAME_BUNDLE_PLAINTEXT_BYTES);
    assert_eq!(
        CONNECTION_FRAME_BUNDLE_CIPHERTEXT_BYTES,
        CONNECTION_FRAME_BUNDLE_PLAINTEXT_BYTES + XCHACHA20_POLY1305_TAG_BYTES
    );
    assert_eq!(
        Ciphertext::<CONNECTION_FRAME_BUNDLE_CIPHERTEXT_BYTES>::DATA_LEN,
        CONNECTION_FRAME_BUNDLE_CIPHERTEXT_BYTES
    );
    assert_eq!(
        Ciphertext::<CONNECTION_FRAME_BUNDLE_CIPHERTEXT_BYTES>::LEN,
        U32_BYTES + CONNECTION_FRAME_BUNDLE_CIPHERTEXT_BYTES
    );
    assert_eq!(
    ...
    assert_eq!(
        Ciphertext::<CONNECTION_FRAME_FILE_SLICE_CIPHERTEXT_BYTES>::DATA_LEN,
        CONNECTION_FRAME_FILE_SLICE_CIPHERTEXT_BYTES
    );
    assert_eq!(
        Ciphertext::<CONNECTION_FRAME_FILE_SLICE_CIPHERTEXT_BYTES>::LEN,
        U32_BYTES + CONNECTION_FRAME_FILE_SLICE_CIPHERTEXT_BYTES
    );
    assert!(CONNECTION_FRAME_FILE_SLICE_CIPHERTEXT_BYTES <= u32::MAX as usize);
}
