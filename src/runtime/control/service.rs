...
pub fn open_db_load(
    ...
    // Stage 3.5 (codex plan): the daemon hosts one endpoint and zero or
    // more workspace bindings. For legacy CLI/RPC callers that still need
    // a `recorded_by` value, derive it from the workspace bindings — which
    // is what `recorded_by` always actually meant in practice (the
    // workspace's primary peer at the time the invite was accepted).
    let bindings = crate::db::transport_creds::list_hosted_workspaces(&conn)?;
    // Distinct legacy_peer_id; treat each as a tenant for the legacy code path.
    let mut scoped_peers: Vec<String> = bindings.into_iter().map(|b| b.legacy_peer_id).collect();
    scoped_peers.sort();
    scoped_peers.dedup();
    ...
    // Fresh DB / pre-workspace state: there's no legacy transport identity
    // to fall back on — the QUIC path is gone.
...
///
/// Stage 3.5: this is now `WorkspaceBinding`-driven. Each row maps a hosted
/// workspace to the `recorded_by` value that the legacy projector path uses;
/// callers that want pure endpoint+workspace data should use
/// `load_local_endpoint` / `list_hosted_workspaces` directly.
pub fn svc_node_status(db_path: &str) -> ServiceResult<Vec<NodeTenantItem>> {
    ...
    Ok(bindings
        .into_iter()
        .map(|b| NodeTenantItem {
            peer_id: b.legacy_peer_id,
...
pub async fn svc_intro(
    ...
    _peer_b: &str,
    _ttl_ms: u64,
    _attempt_window_ms: u32,
) -> ServiceResult<bool> {
    // Phase #12: the legacy peering hole-punch / intro workflow has been
    // deleted. The new ControlLoopRuntime path does not yet implement
    // intro/punch; the RPC surface is preserved but returns an error.
    Err(ServiceError(
        "intro/holepunch is not implemented on the ControlLoopRuntime path yet".to_string(),
    ))
}
...
/// Resolve the tenant's workspace via invites_accepted and read all keys
/// for the workspace — a sibling tenant's projection is reused via the
/// global PK.
fn load_key_secrets(
    db: &rusqlite::Connection,
    recorded_by: &str,
    ...
    let workspace_id_b64 = match workspace_id_b64 {
        Some(v) => v,
        None => return key_secrets,
    };
    if let Ok(mut stmt) =
        db.prepare("SELECT event_id, key_bytes FROM key_secrets WHERE workspace_id = ?1")
    {
        if let Ok(rows) = stmt.query_map(rusqlite::params![&workspace_id_b64], |row| {
...
mod tests {
    ...
    fn test_open_db_load_resolves_scoped_peer_with_multiple_local_identities() {
        ...
        let (bootstrap_cert, bootstrap_priv) =
            crate::runtime::legacy_identity::generate_self_signed_cert_from_signing_key(&bootstrap_key).unwrap();
        let bootstrap_fp =
            crate::runtime::legacy_identity::extract_spki_fingerprint(bootstrap_cert.as_ref()).unwrap();
