...
//!
//! Rows are keyed by `workspace_id || content_event_id` so callers can scan a
...
pub fn content_event_key(workspace_id: &WorkspaceId, event_id: &[u8; 32]) -> Vec<u8> {
...
pub fn content_event_row(event_id: [u8; 32], fact: &ContentEventFact) -> Result<TableRow, String> {
    ...
    Ok(TableRow {
        ...
        key: content_event_key(&fact.workspace_id, &event_id),
        value,
    })
}
