...
/// Row value layout: `created_at_ms(8) || user_authority_event_id(32) ||
/// user_invite_event_id_or_zero(32) || public_key(32)`.
...
pub fn encode_fact(fact: &DeviceInviteFact) -> Result<Vec<u8>, String> {
    ...
    out[73..105].copy_from_slice(&fact.user_invite_event_id.unwrap_or([0; 32]));
...
pub fn decode_fact(bytes: &[u8]) -> Result<DeviceInviteFact, String> {
    ...
    let user_invite_event_id = if user_invite_raw == [0; 32] {
        None
    } else {
...
pub(crate) fn encode_row_value(fact: &DeviceInviteFact) -> Result<Vec<u8>, String> {
    ...
    out[8..40].copy_from_slice(&fact.user_authority_event_id);
    out[40..72].copy_from_slice(&fact.user_invite_event_id.unwrap_or([0; 32]));
...
pub(crate) fn decode_row_value(value: &[u8]) -> Result<DecodedRowValue, String> {
    ...
    let mut user_authority_event_id = [0; 32];
    user_authority_event_id.copy_from_slice(&value[8..40]);
    let mut user_invite_raw = [0; 32];
...
pub(crate) struct DecodedRowValue {
    ...
    pub user_invite_event_id: Option<[u8; 32]>,
