...
use crate::core::projection::{MatchedContext, ProjectionContext, ProjectionOutput};

use super::super::fact::{EventId, KeyWrapId};
use super::super::layout;
use super::super::matchers;
use super::validation::{require_fact_scope, validate_sync_fact_workspace};

pub(super) fn project_sync_range_request(
    fact: &Fact,
    ...
    let range_need = matchers::range_event_need(fact.id, scope.clone(), request.start, request.end);
    ...
    for root in roots {
        ...
        let dep_ready = has_matched_exact_event(projection_context, &dep_need, root.dependency_id)?;
        ...
        // Incomplete roots are still useful: they tell the matcher which
        // precise dependency or key-wrap facts would make the original range
        // request ready. Keep the range need while any root is incomplete so a
        // later exact match still reloads the root context needed to send.
...
fn validate_range_match(matched: &MatchedContext) -> Result<matchers::RangeOfferSelector, String> {
    ...
    let root = layout::decode_encrypted_root(&matched.payload.bytes)?;
    validate_sync_fact_workspace(&matched.payload, root.workspace_id)?;
...
fn exact_event_id_from_payload(fact: &Fact, expected: EventId) -> Result<EventId, String> {
    match fact.bytes.first().copied() {
        Some(layout::TYPE_ENCRYPTED_ROOT) => {
            let root = layout::decode_encrypted_root(&fact.bytes)?;
            validate_sync_fact_workspace(fact, root.workspace_id)?;
            Ok(root.event_id)
        }
        Some(layout::TYPE_SHARED_EVENT) => {
            let shared = layout::decode_shared_event(&fact.bytes)?;
            validate_sync_fact_workspace(fact, shared.workspace_id)?;
            Ok(shared.event_id)
        }
        Some(layout::TYPE_KEY_WRAP_AVAILABLE) => {
            let key = layout::decode_key_wrap_available(&fact.bytes)?;
            validate_sync_fact_workspace(fact, key.workspace_id)?;
            Ok(key.key_wrap_id)
        }
