...
use super::fact::{DependencyFact, EncryptedRootFact, KeyOfferFact, SyncRangeRequestFact};

pub const TYPE_SYNC_RANGE_REQUEST: u8 = 160;
pub const TYPE_ENCRYPTED_ROOT: u8 = 161;
pub const TYPE_DEPENDENCY: u8 = 162;
pub const TYPE_KEY_OFFER: u8 = 163;

pub const SYNC_RANGE_REQUEST_BYTES: usize = 1 + 32 + 32 + 8 + 8;
pub const ENCRYPTED_ROOT_BYTES: usize = 1 + 32 + 32 + 32;
pub const DEPENDENCY_BYTES: usize = 1 + 32 + 32;
pub const KEY_OFFER_BYTES: usize = 1 + 32 + 32;
...
pub fn encode_encrypted_root(fact: &EncryptedRootFact) -> Result<Vec<u8>, String> {
    ...
    out[1..33].copy_from_slice(&fact.workspace_id);
    out[33..65].copy_from_slice(&fact.dependency_id);
    out[65..97].copy_from_slice(&fact.key_id);
    Ok(out)
}
...
pub fn decode_encrypted_root(bytes: &[u8]) -> Result<EncryptedRootFact, String> {
    ...
    Ok(EncryptedRootFact {
        workspace_id: bytes[1..33].try_into().unwrap(),
        dependency_id: bytes[33..65].try_into().unwrap(),
        key_id: bytes[65..97].try_into().unwrap(),
    })
}

pub fn encode_dependency(fact: &DependencyFact) -> Result<Vec<u8>, String> {
    let mut out = vec![0; DEPENDENCY_BYTES];
    wire::put_u8(TYPE_DEPENDENCY, &mut out[0..1]).map_err(wire_err)?;
    out[1..33].copy_from_slice(&fact.workspace_id);
    out[33..65].copy_from_slice(&fact.event_id);
    Ok(out)
}

pub fn decode_dependency(bytes: &[u8]) -> Result<DependencyFact, String> {
    wire::expect_len(bytes, DEPENDENCY_BYTES).map_err(wire_err)?;
    expect_tag(bytes, TYPE_DEPENDENCY, "sync dependency")?;
    Ok(DependencyFact {
        workspace_id: bytes[1..33].try_into().unwrap(),
...
pub fn encode_key_offer(fact: &KeyOfferFact) -> Result<Vec<u8>, String> {
    let mut out = vec![0; KEY_OFFER_BYTES];
    wire::put_u8(TYPE_KEY_OFFER, &mut out[0..1]).map_err(wire_err)?;
    out[1..33].copy_from_slice(&fact.workspace_id);
    out[33..65].copy_from_slice(&fact.key_id);
    Ok(out)
}

pub fn decode_key_offer(bytes: &[u8]) -> Result<KeyOfferFact, String> {
    wire::expect_len(bytes, KEY_OFFER_BYTES).map_err(wire_err)?;
    expect_tag(bytes, TYPE_KEY_OFFER, "sync key offer")?;
    Ok(KeyOfferFact {
        workspace_id: bytes[1..33].try_into().unwrap(),
