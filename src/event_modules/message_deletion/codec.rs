...
pub struct MessageDeletionEvent {
    ...
    /// Canonical workspace_id this deletion belongs to. Set by callers
    /// from the local authoring context (poc-9 plan.md Stage 2 — every
    /// event carries its own `workspace_id`). The projector uses this
    /// to scope the `deleted` label and the deletion intent rows.
...
/// MessageDeletion (type 7): type(1) + created_at(8) + workspace_id(32) + target_event_id(32)
///                          + author_id(32) + signed_by(32) + signer_type(1) + signature(64) = 202
pub const MESSAGE_DELETION_WIRE_SIZE: usize = wire_size_for_fields(MESSAGE_DELETION_FIELDS);

/// Wire format (202 bytes fixed, signed):
/// [0]       type_code = 7
/// [1..9]    created_at_ms (u64 LE)
/// [9..41]   workspace_id (32 bytes)
/// [41..73]  target_event_id (32 bytes)
/// [73..105] author_id (32 bytes)
...
pub fn encode_message_deletion(event: &ParsedEvent) -> Result<Vec<u8>, EventError> {
    ...
    let values = vec![
        ...
        FieldValue::EventId(del.workspace_id),
