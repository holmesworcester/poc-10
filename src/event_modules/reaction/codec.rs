...
/// Reaction (type 2): type(1) + created_at(8) + workspace_id(32) + target_event_id(32) + author_id(32)
///                   + emoji(64) + signed_by(32) + signer_type(1) + signature(64) = 266
///
/// `workspace_id` is the canonical identifier of the workspace this reaction
/// belongs to (poc-9 plan.md Stage 2 — every event carries its own
/// `workspace_id`). The projector reads this directly and drops the
/// thread-local `current_recorded_by` bridge for chain-friendly callers.
...
pub struct ReactionEvent {
    ...
    /// Canonical workspace_id this reaction belongs to. Filled in by callers
    /// from the local authoring context; the projector reads it to write
    /// `reactions.workspace_id` and to scope label lookups.
...
pub fn parse_reaction(blob: &[u8]) -> Result<ParsedEvent, EventError> {
    ...
    Ok(ParsedEvent::Reaction(ReactionEvent {
        ...
        target_event_id: values[2].as_event_id().unwrap(),
        author_id: values[3].as_event_id().unwrap(),
        emoji: values[4].as_text().unwrap().to_string(),
...
pub fn encode_reaction(event: &ParsedEvent) -> Result<Vec<u8>, EventError> {
    ...
    let values = vec![
        ...
        FieldValue::EventId(rxn.workspace_id),
