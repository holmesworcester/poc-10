...
//! `SendBootstrapRequest` intent has not been wired into transit handlers yet
...
pub fn encode_fact(fact: &InviteSecretFact) -> Result<Vec<u8>, String> {
    ...
    out[97..129].copy_from_slice(&fact.invite_event_id.unwrap_or([0; 32]));
    Ok(out)
}
...
pub fn decode_fact(bytes: &[u8]) -> Result<InviteSecretFact, String> {
    ...
    let invite_event_id = optional_id(invite_raw);
    InviteSecretFact {
        bootstrap_hash,
        bootstrap_secret,
        workspace_id,
...
pub(crate) fn encode_row_value(fact: &InviteSecretFact) -> Result<Vec<u8>, String> {
    ...
    out[64..96].copy_from_slice(&fact.invite_event_id.unwrap_or([0; 32]));
    Ok(out)
}
...
pub(crate) fn decode_row_value(value: &[u8]) -> Result<DecodedRowValue, String> {
    ...
    Ok(DecodedRowValue {
        ...
        invite_event_id: optional_id(invite_raw),
    })
}
...
pub(crate) struct DecodedRowValue {
    ...
    pub invite_event_id: Option<[u8; 32]>,
}
...
mod tests {
    ...
    fn invite_secret_fact_roundtrips_scoped() {
        ...
        assert_eq!(decoded.invite_event_id, Some([2; 32]));
    }
