...
/// PeerShared (type 16):
/// type(1) + created_at(8) + public_key(32) + workspace_id(32) + user_event_id(32)
/// + device_name(64) + signed_by(32) + signer_type(1) + signature(64) = 266
///
/// Plan.md Stage 2: `workspace_id` was implicit (carried by `recorded_by`
/// thread-local) under the legacy chain. Phase 32 makes it explicit so the
/// projector keys writes by `(workspace_id, event_id)` without thread-locals.
/// The `public_key` field stays at offset 9..41 so
...
pub struct PeerSharedEvent {
    ...
    /// Workspace this peer record belongs to. Children/siblings carry the
    /// same `workspace_id`; projection key is `(workspace_id, event_id)`.
...
pub fn parse_peer_shared(blob: &[u8]) -> Result<ParsedEvent, EventError> {
    ...
    let signer_type = values[6].as_u8().unwrap();
    if signer_type != 3 {
...
mod layout_tests {
    ...
    fn parse_peer_shared_rejects_wrong_signer_type() {
        let signer_type_offset = field_offset(PEER_SHARED_FIELDS, 6);
