...
/// Ensure a local `tenant` event exists for this endpoint and return its event id.
/// `tenant` is the local root for local-only identity events.
///
/// Stage 3.5 round-9 step 3: `tenants` is keyed by `event_id` alone (the
/// table is moving toward an endpoint singleton). Until the endpoint-model
/// migration completes we still need per-recorded_by isolation for callers
/// that bootstrap multiple local tenants in a single DB (legacy
/// multi-tenant create-workspace flows). We dedupe via `valid_events`
/// (still recorded_by-keyed) and fall back to creating a fresh tenant
/// event when this recorded_by has no projected tenant root yet.
...
fn create_deterministic_key_secret_event(
    ...
    let expected = crate::event_modules::key_secret::deterministic_key_secret_event_id(
        workspace_id,
        &key_bytes,
    );
    let sk_evt =
        crate::event_modules::key_secret::deterministic_key_secret_event(*workspace_id, key_bytes);
...
fn recent_key_rotations_for_peer(
    ...
    // plan.md Stage 2: read by workspace_id — the new key for both
    // key_rotations and key_secrets. Resolve the tenant's workspace via
    // invites_accepted, then return rotations that have a paired
    // key_secret (i.e. local key material is available for emission).
    let workspace_id_b64: Option<String> = conn
        .query_row(
            "SELECT workspace_id
        ...
        .optional()?;
    let workspace_id_b64 = match workspace_id_b64 {
        Some(v) => v,
    ...
    let mut stmt = conn.prepare(
        "SELECT kr.key_event_id,
         ...
         JOIN key_secrets ks
           ON ks.workspace_id = kr.workspace_id
    ...
    let rows = stmt.query_map(rusqlite::params![&workspace_id_b64, limit as i64], |row| {
        Ok((
            row.get::<_, String>(0)?,
            row.get::<_, u8>(1)?,
            row.get::<_, String>(2)?,
...
fn current_removal_frontier_for_peer(
    ...
    // Plan.md round-9 step 1: `removals` is now keyed by
    // `(workspace_id, event_id)` — resolve the caller's workspace_id
    // from `invites_accepted` first, then frontier-scan within it.
    let workspace_id_b64: Option<String> = conn
        .query_row(
            "SELECT workspace_id
             FROM invites_accepted
    ...
    let rows = stmt.query_map(rusqlite::params![&workspace_id_b64], |row| {
        row.get::<_, String>(0)
    })?;
    let mut out = Vec::new();
    for row in rows {
...
fn latest_materialized_key_for_peer(
    ...
    // plan.md Stage 2: filter by workspace_id (resolved via invites_accepted).
    let workspace_id_b64: Option<String> = conn
        .query_row(
            "SELECT workspace_id
             FROM invites_accepted
...
fn latest_content_key_for_frontier(
    ...
    // plan.md Stage 2: read by workspace_id (resolved via invites_accepted).
    let workspace_id_b64: Option<String> = conn
        .query_row(
            "SELECT workspace_id
             FROM invites_accepted
...
fn load_key_secret_bytes(
    ...
    let _ = recorded_by;
    // plan.md Stage 2: key_secrets PK is now (workspace_id, event_id);
    // a single row is global per-workspace. Look up by event_id alone —
    // the bridge writes recorded_by, the chain writes "__chain__", so we
    // must not filter by recorded_by anymore.
...
fn key_shared_exists_for_delivery_target(
    ...
    // plan.md Stage 2: key_shared PK is (workspace_id, event_id). The
    // semantic identity of a delivery target (key_event_id, frontier_hash,
    // delivery_target_id, recipient_event_id) is workspace-scoped already,
    // so we no longer filter by recorded_by.
...
pub(crate) fn rotate_content_key_for_peer(
    ...
    // Resolve workspace_id from the tenant's invites_accepted row. This
    // avoids the full identity chain dependency that
    // `load_local_authoring_context` requires (peer_secret, peers_shared,
    // user) — those checks are deferred to the share-emit call below.
    let workspace_id = super::queries::resolve_workspace_for_peer(conn, recorded_by)?;
    let key_event_id = create_deterministic_key_secret_event(
        conn,
        recorded_by,
        &workspace_id,
        content_key_bytes,
    )?;
    let rotation_event_id = ensure_rotation_for_key_frontier(
        conn,
        recorded_by,
        &workspace_id,
        &key_event_id,
...
pub(crate) fn expected_invite_bootstrap_spki_from_invite_key(
    ...
    Ok(crate::crypto::spki_fingerprint_from_ed25519_pubkey(
...
pub(crate) fn ensure_content_key_for_peer(
    ...
    if frontier_refs.is_empty() {
        if let Some(existing) = latest_materialized_key_for_peer(conn, recorded_by)? {
            let authoring =
                crate::event_modules::workspace::load_local_authoring_context(conn, recorded_by)?;
            let _ = ensure_rotation_for_key_frontier(
                conn,
                recorded_by,
...
pub(crate) fn wrap_content_key_for_invite(
    ...
    let invite_secret_event_id =
        store_invite_secret(conn, recorded_by, invite_event_id, workspace_id, invite_key)?;
