...
fn create_user_invite_materializes_pending_bootstrap_trust_via_projection() {
    ...
    // Wave 1: admin events dropped; use the user_event_id from peers_shared
    // as the authority stand-in that resolve_admin_event_for_signer would now
    // surface in production.
    let admin_event_id: EventId = conn
        .query_row(
            "SELECT user_event_id FROM peers_shared WHERE recorded_by = ?1 LIMIT 1",
            rusqlite::params![&recorded_by],
            |row| row.get::<_, String>(0),
        )
        .ok()
        .and_then(|b64| crate::crypto::event_id_from_base64(&b64))
        .expect("workspace bootstrap must create a peer_shared event");
...
fn create_workspace_allows_unscoped_recorded_by_when_creds_exist_and_creates_new_tenant() {
    ...
    let (cert, key) = crate::runtime::legacy_identity::generate_self_signed_cert().expect("generate cert");
    let fp = crate::runtime::legacy_identity::extract_spki_fingerprint(cert.as_ref()).expect("extract spki");
...
fn send_rotates_on_new_local_removal_frontier_and_reuses_frontier_key() {
    ...
    // dropped — query via the caller's workspace resolved through
    // invites_accepted.
    let stored_frontier_hash: String = conn
        .query_row(
            "SELECT kr.frontier_hash
             FROM key_rotations kr
             JOIN invites_accepted ia
               ON ia.workspace_id = kr.workspace_id
             WHERE ia.recorded_by = ?1 AND kr.key_event_id = ?2
