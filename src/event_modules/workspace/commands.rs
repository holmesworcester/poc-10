...
use crate::event_modules::{
    peer_secret::PeerSecretEvent, DeviceInviteEvent, InviteAcceptedEvent, ParsedEvent,
...
fn replay_existing_workspace_shared_events_for_tenant(
    ...
    //
    // plan.md Stage 2 (key-family chain-friendly migration): KeyShared
    // / KeyRequest / KeyRotation are all `Shared` scope and therefore
    // already match this filter — what changed is that their projection
    // PK migrated from `(recorded_by, event_id)` to `(workspace_id,
    // event_id)`, which means a sibling tenant's projection of e.g. a
    // KeyShared row is now globally readable and the new joiner can
    // cascade off it (the joiner's invite_secret triggers the projector's
    // unwrap path, which emits the deterministic key_secret locally).
    //
    // KeySecret itself is `Local` scope and intentionally NOT replayed
    // here: cross-tenant key-history disclosure must flow through the
    // KeyShared unwrap path so that history caps and invite-target
    // policies still apply. A new joiner derives its own key_secret rows
    // via the projector's deterministic-blob emission when a matching
    // KeyShared replays.
...
fn create_workspace_inner(
    ...
    let ws = ParsedEvent::Workspace(WorkspaceEvent {
        created_at_ms: current_timestamp_ms_u64(),
        // Workspace events are roots: by convention, `workspace_id` is the
        // workspace's public_key. The event_id (BLAKE3 of canonical bytes)
        // is what other events reference as `workspace_id`, so the field
        // here is the local label for the root.
    ...
    // 8. Admin event dropped in Wave 1; the creator user grants are now
    // implicit (validated through the user_invite/workspace chain).
    let _ = ws_eid;
    ...
    emit_peer_secret(db, &derived_peer_id, &ws_eid, &psf_eid, &peer_shared_key)?;
...
pub fn join_workspace_as_new_user(
    ...
    // Joining is always a new local tenant; if the workspace is already
    // hosted on this daemon, reject before any event is written.
...
fn join_workspace_inner(
    ...
    let _ = ops::store_invite_secret(db, recorded_by, invite_event_id, &workspace_id, invite_key)?;
...
pub fn add_device_to_workspace(
    ...
    // Plan.md line 114: at most one workspace instance per daemon endpoint.
    // A device-link accept mints a new PeerShared and therefore a new
    // local tenant binding. Reject if the daemon already hosts this
    // workspace under any tenant.
    super::uniqueness::assert_endpoint_can_host(db, &workspace_id)
        .map_err(|e| -> Box<dyn std::error::Error + Send + Sync> { e.to_string().into() })?;
    ...
    let _ = ops::store_invite_secret(db, recorded_by, device_invite_event_id, &workspace_id, device_invite_key)?;
