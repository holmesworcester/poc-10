...
pub fn list(db: &Connection, recorded_by: &str) -> Result<Vec<WorkspaceRow>, rusqlite::Error> {
    // Plan.md Stage 2: `workspaces` is now keyed by `(workspace_id,
    // event_id)` — global, not per-tenant. To preserve the legacy
    // per-tenant `list` semantics (CLI flow shows only workspaces this
    // tenant has joined), restrict via `invites_accepted` (still keyed
    // on `recorded_by` in Stage 2).
    let mut stmt = db.prepare(
        "SELECT w.event_id, w.workspace_id, COALESCE(w.name, '')
         FROM workspaces w
         WHERE EXISTS (
...
pub fn name(db: &Connection, recorded_by: &str) -> Result<String, rusqlite::Error> {
    ...
    // Match `list`: scope via invites_accepted, not via the workspaces
    // recorded_by column (which is now nullable / shadow-only).
    Ok(db
        .query_row(
            "SELECT COALESCE(w.name, '')
             FROM workspaces w
             JOIN invites_accepted ia
               ON ia.workspace_id = w.workspace_id
             WHERE ia.recorded_by = ?1
             LIMIT 1",
...
pub fn keys(
    ...
    // Wave 1: admin event dropped; admin_count is always 0 and admins list is empty.
    let admin_count = 0;
...
pub fn content_keys(
    ...
    // plan.md Stage 2: key_secrets PK migrated to (workspace_id, event_id);
    // resolve the tenant's workspace_id via invites_accepted and filter by
    // it.
    let workspace_id_b64: Option<String> = db
        .query_row(
            "SELECT workspace_id
             FROM invites_accepted
             WHERE recorded_by = ?1
    ...
    let workspace_id_b64 = match workspace_id_b64 {
        Some(v) => v,
        None => {
            return Ok(ContentKeysResponse {
                key_secret_count: 0,
                latest_key_event_id: None,
                keys: Vec::new(),
            })
        }
    };
...
fn list_view_users(db: &Connection, recorded_by: &str) -> Result<Vec<ViewUser>, rusqlite::Error> {
    // peers_shared is now globally keyed by (workspace_id, event_id);
    // restrict to the active tenant's workspace via invites_accepted.
    let mut stmt = db.prepare(
...
fn tenant_workspace_name(
    ...
    // Plan.md Stage 2: workspaces is keyed on (workspace_id,
    // event_id), so scoping via invites_accepted (still keyed by
    // recorded_by) is the per-tenant filter. Caller still provides
    // workspace_id for the join target.
    Ok(db
        .query_row(
            "SELECT COALESCE(w.name, '')
             FROM workspaces w
...
fn tenant_local_username(db: &Connection, recorded_by: &str) -> Result<String, rusqlite::Error> {
    // Plan.md round-9 step 5A: peers_shared and users no longer carry a
    // `recorded_by` shadow column. Per-tenant scoping resolves the
    // caller's workspace_id via invites_accepted (still keyed on
    // `recorded_by` in Stage 2) and filters by workspace_id.
...
mod tests {
    ...
    fn insert_ready_fixture(
        ...
        conn.execute(
            "INSERT INTO peers_shared
             (recorded_by, event_id, workspace_id, public_key, transport_fingerprint, user_event_id, device_name)
        ...
        if ready {
            // Plan.md round-9 step 4: peer_secrets keyed by (workspace_id, peer_id);
            // secret_event_id is UNIQUE.
            conn.execute(
                "INSERT INTO peer_secrets
                 (workspace_id, peer_id, secret_event_id, signer_event_id, private_key, created_at)
                 VALUES (?1, ?2, ?3, ?4, ?5, 1)",
                rusqlite::params![
                    workspace_id,
                    recorded_by,
    ...
    fn view_separates_top_level_tenants_from_workspace_user_devices() {
        ...
        conn.execute(
            "INSERT INTO users (recorded_by, event_id, workspace_id, public_key, username)
             VALUES
             ('tenant-a', 'user-alice', 'ws-design', X'01', 'alice'),
             ('tenant-a', 'user-carol', 'ws-design', X'02', 'carol'),
             ('tenant-b', 'user-remote', 'ws-ops', X'03', 'remote-first'),
             ('tenant-b', 'user-bob', 'ws-ops', X'04', 'bob')",
            [],
        )
        .unwrap();
        ...
        conn.execute(
            "INSERT INTO peers_shared
             (recorded_by, event_id, workspace_id, public_key, transport_fingerprint, user_event_id, device_name)
             VALUES
             ('tenant-a', 'peer-alice-phone', 'ws-design', X'11', ?1, 'user-alice', 'phone'),
             ('tenant-a', 'peer-carol-workstation', 'ws-design', X'12', ?2, 'user-carol', 'workstation'),
             ('tenant-b', 'peer-remote', 'ws-ops', X'13', ?3, 'user-remote', 'shared-box'),
             ('tenant-b', 'peer-bob-laptop', 'ws-ops', X'14', ?4, 'user-bob', 'laptop')",
            rusqlite::params![
                vec![0x01u8; 32],
                vec![0x09u8; 32],
                vec![0x08u8; 32],
