...
/// Layout: `type(1) || workspace_id(32) || invite_event_id(32) ||
/// invite_secret_event_id(32) || bootstrap_hash(32) || accepted_endpoint_id(32)`.
pub const FACT_BYTES: usize = 1 + 32 + 32 + 32 + 32 + 32;
/// Row value layout: `invite_accepted_event_id(32) || invite_secret_event_id(32) ||
/// bootstrap_hash(32)`.
...
pub fn encode_fact(fact: &InviteAcceptedFact) -> Result<Vec<u8>, String> {
    ...
    out[33..65].copy_from_slice(&fact.invite_event_id);
    out[65..97].copy_from_slice(&fact.invite_secret_event_id);
...
pub fn decode_fact(bytes: &[u8]) -> Result<InviteAcceptedFact, String> {
    ...
    invite_event_id.copy_from_slice(&bytes[33..65]);
    let mut invite_secret_event_id = [0; 32];
    invite_secret_event_id.copy_from_slice(&bytes[65..97]);
...
pub(crate) fn encode_row_value(
    ...
    out[0..32].copy_from_slice(invite_accepted_event_id);
    out[32..64].copy_from_slice(&fact.invite_secret_event_id);
...
pub(crate) fn decode_row_value(value: &[u8]) -> Result<DecodedRowValue, String> {
    ...
    let mut invite_accepted_event_id = [0; 32];
    invite_accepted_event_id.copy_from_slice(&value[0..32]);
    let mut invite_secret_event_id = [0; 32];
    invite_secret_event_id.copy_from_slice(&value[32..64]);
    let mut bootstrap_hash = [0; 32];
...
mod tests {
    ...
    fn row_value_roundtrip() {
        ...
        assert_eq!(decoded.invite_accepted_event_id, [9; 32]);
        assert_eq!(decoded.invite_secret_event_id, [3; 32]);
