...
/// `device_invites` projection table.
///
...
/// `device_invites` resolve the caller's workspace_id from the
...
pub fn ensure_schema(conn: &Connection) -> rusqlite::Result<()> {
    let needs_recreate = match table_state(conn, "device_invites")? {
        Some(state) => {
    ...
    if needs_recreate {
        conn.execute_batch("DROP TABLE IF EXISTS device_invites")?;
    }
    conn.execute_batch(
        "
        ...
        CREATE INDEX IF NOT EXISTS idx_device_invites_event_id
            ON device_invites(event_id);
        ",
    )?;
    Ok(())
}
