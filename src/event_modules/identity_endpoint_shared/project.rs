...
use crate::event_modules::identity_device_invite::layout as device_invite_layout;
    ...
    fn project(
        ...
        Ok(ProjectionOutput::new()
            ...
            .offer(sealed_message::matchers::signer_offer(
                fact.id,
                sealed_message::matchers::workspace_scope(event.workspace_id),
                event.endpoint_id,
            ))
...
fn authority_need(
    ...
    match event.endpoint_role {
        EndpointRole::InviteServer => identity_matchers::exact_need(
            fact.id,
            identity_matchers::invite_server_role(),
            signer_id,
        ),
        EndpointRole::Device => identity_matchers::exact_need(
            fact.id,
...
mod projector_tests {
    ...
    fn endpoint_shared_projector_materializes_row_with_invite_server_context() {
        ...
        let context = ProjectionContext::from_matches(vec![MatchedContext {
            ...
            offer: identity_matchers::invite_server_offer(invite_server_fact.id),
        ...
        assert!(output
            .offers
            .iter()
            .any(|offer| offer.role == identity_matchers::endpoint_shared_role()));
    ...
    fn signed_device_invite_fact(
        ...
        let payload = topo::event_modules::identity_device_invite::layout::encode_fact(
            &topo::event_modules::identity_device_invite::fact::DeviceInviteFact {
                created_at_ms: 1,
                workspace_id,
