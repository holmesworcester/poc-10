...
//!
//! Rows are keyed by `connection_id || fact_id` so transit handlers can scan
//! all advertisements queued for a connection. The value stores the
//! timestamp and the advertised event id; the fact id in the key keeps
//! distinct advertisements distinct even when the same event id is
//! re-advertised from a later range compare.

use crate::core::store::{TableName, TableRow};
use crate::core::wire;

use super::fact::{ConnectionId, EventId, SyncHaveIdFact};
...
pub fn sync_have_id_row(fact_id: [u8; 32], fact: &SyncHaveIdFact) -> Result<TableRow, String> {
    let mut value = vec![0; ROW_VALUE_BYTES];
    wire::put_u64be(fact.timestamp, &mut value[0..8]).map_err(wire_err)?;
    value[8..40].copy_from_slice(&fact.event_id);
    Ok(TableRow {
        table: SYNC_HAVE_ID_ROWS,
        key: sync_have_id_key(&fact.connection_id, &fact_id),
        value,
    })
}
...
pub fn decode_sync_have_id_row(key: &[u8], value: &[u8]) -> Result<SyncHaveIdRow, String> {
    ...
    event_id.copy_from_slice(&value[8..40]);
    Ok(SyncHaveIdRow {
        connection_id,
        fact_id,
        timestamp,
