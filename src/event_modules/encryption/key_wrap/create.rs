//! Local constructors and unwrap helpers for key-wrap facts.

use super::super::recipient_key::context as recipient_context;
use super::super::recipient_key::fact::{LocalRecipientKeyFact, RecipientKeyFact};
use super::super::wrap_source::context::WrapSourceKind;
...
use crate::event_modules::encryption::local_secret::fact::{
    LocalHistoryNodeSecretFact, LocalKeySecretFact,
};
use crate::event_modules::encryption::local_secret::layout as local_secret_layout;
use crate::event_modules::encryption::recipient_key::layout as recipient_key_layout;
...
pub fn materialize_key_wrap_fact(
    ...
    Ok(Fact::new(
        recipient_context::workspace_scope(intent.workspace_id),
        wrap.created_at_ms,
...
pub fn unwrap_key_wrap_fact(
    ...
    let local = recipient_key_layout::decode_local_recipient_key(&local_recipient_key_fact.bytes)?;
...
fn wrap_material(
    ...
    match intent.source {
        WrapSourceKind::FrontierRoot => {
            let source = local_secret_layout::decode_local_key_secret(&source_fact.bytes)?;
        ...
        WrapSourceKind::HistoryNode {
            ...
            let source =
                local_secret_layout::decode_local_history_node_secret(&source_fact.bytes)?;
            require_source_workspace_and_frontier(intent, source.workspace_id, source.frontier_id)?;
...
fn root_secret_fact(
    ...
    Ok(Fact::new(
        ...
        wrap.created_at_ms,
        local_secret_layout::encode_local_key_secret(&LocalKeySecretFact {
...
fn history_secret_fact(
    ...
    Ok(Fact::new(
        crate::core::facts::FactScope::Local,
        wrap.created_at_ms,
        local_secret_layout::encode_local_history_node_secret(&LocalHistoryNodeSecretFact {
            workspace_id: wrap.workspace_id,
            frontier_id: wrap.frontier_id,
            owner_endpoint_id: wrap.signer_endpoint_id,
