...
use crate::event_modules::{identity_endpoint_shared, sealed_message, signed_fact};

use super::layout;
use super::matchers::{self, WrapSourceKind, WrapSourceSelector};
...
pub(super) fn has_matching_signer_public_key(
    ...
    projection_context
        ...
        .any(|matched| {
            if let Ok(signer) = sealed_message::layout::decode_signer_pubkey(&matched.payload.bytes)
            {
                return signer.signer_id.as_slice() == need.selector.as_bytes()
                    && signer.public_key == *signer_public_key;
            }
            let Ok(envelope) = signed_fact::layout::decode_signed_fact(&matched.payload.bytes)
            else {
                return false;
            };
            ...
            let Ok(endpoint) = identity_endpoint_shared::layout::decode_fact(&envelope.payload)
            else {
                return false;
            };
    ...
    match source.kind {
        ...
        WrapSourceKind::HistoryNode {
            ...
            if node.workspace_id != source.workspace_id
                ...
                || node.event_id_prefix != event_id_prefix
            {
