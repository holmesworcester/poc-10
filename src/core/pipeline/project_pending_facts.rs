...
//!
//! Projection is intentionally sequential by fact, but durable inputs may share
//! a bounded SQLite transaction. Inside that transaction each fact is loaded,
//! projected, and committed before the next fact is prepared, so later facts can
//! read context and rows written by earlier facts in the same batch. The batch
//! transaction is the durable boundary that clears pending rows, replaces
//! context and time wakes, wakes newly matched dependent facts, and commits
//! projector shared effects. If scheduling or projection atomicity changes,
...
/// Process pending durable facts and ephemeral inputs from SQLite until there
/// is no work or `limit` inputs have completed projection.
///
...
///
/// The important ordering rule is load -> prepare -> commit for each fact
/// before moving to the next fact. This keeps the batch atomic while preserving
/// the projector model: same-batch facts that depend on earlier facts can read
/// their newly committed context through the same transaction handle.
...
///
/// Ephemeral inputs still use this path. Durable facts use
/// `process_pending_fact_in_tx` inside a bounded batch transaction so later
/// facts can read earlier same-batch writes.
