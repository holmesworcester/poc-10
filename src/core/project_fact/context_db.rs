...
pub(crate) fn pending_projection_input_context_for_owner(
    ...
    for (need, offer_id) in pairs {
        ...
        push_stored_context_match(&need, offer, &mut seen, &mut matched);
    }
...
fn push_stored_context_match(
    ...
    matched.push(MatchedContext {
        need: need.clone(),
        offer,
        offer_owner_scope: stored.owner_scope,
        offer_owner_received_at: stored.owner_received_at,
    });
}
