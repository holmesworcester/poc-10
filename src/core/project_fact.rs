...
fn prepare_projection(
    ...
    validate_context_offer_values(&projected_context)?;
...
fn validate_context_offer_values(context: &ContextSet) -> Result<(), String> {
    if context.offers.iter().any(|offer| offer.value.is_empty()) {
        return Err("context offers must provide an explicit non-empty semantic value".to_string());
    }
    Ok(())
}
...
pub mod context {
    ...
    /// One matched need/offer pair plus offering-fact metadata.
    ///
    /// Core constructs this from standing context rows before calling the
    /// projector. The context value is `offer.value`; the offering fact's id,
    /// scope, and receive time are provenance, not a hidden retained fact load.
    ...
    pub struct MatchedContext {
        ...
        /// Scope of the fact that emitted the offer.
        pub offer_owner_scope: FactScope,
        /// Receive timestamp of the fact that emitted the offer.
        pub offer_owner_received_at: u64,
    }
    ...
    impl ProjectionContext {
        ...
        /// prefer matched-context lookups when a proof depends on a need.
        ...
        /// Build context from already matched need/offer triples.
        ...
        /// Return the first matched offer for a need, if any.
        pub fn match_for(&self, need: &ContextNeed) -> Option<&MatchedContext> {
            self.matched_entries_for(need).next()
        }

        pub fn match_for_checked(
            &self,
            need: &ContextNeed,
            label: &str,
        ) -> Result<Option<&MatchedContext>, String> {
            let Some(matched) = self.match_for(need) else {
                return Ok(None);
            };
            if matched.offer.value.is_empty() {
                return Err(format!("{label} context offer value is empty"));
            }
            Ok(Some(matched))
        }

        /// Return every matched offer for a need.
    ...
    impl MatchedContext {
        pub fn offer_owner(&self) -> FactId {
            self.offer.owner
        }

        pub fn require_owner(&self, expected: FactId, label: &str) -> Result<(), String> {
            if self.offer.owner != expected {
                return Err(format!("{label} matched offer owner mismatch"));
            }
            Ok(())
        }

        pub fn require_owner_scope(&self, expected: &FactScope, label: &str) -> Result<(), String> {
            if &self.offer_owner_scope != expected {
                return Err(format!("{label} matched offer scope mismatch"));
            }
            Ok(())
        }
...
pub mod effects {
    ...
    pub fn fact_purged_offer(
        ...
        fact_purged_range_offer(owner, scope, key.clone(), key, value)
    }
    ...
    mod tests {
        ...
        fn purge_need_and_offer_match_same_opaque_key() {
            ...
            let offer = fact_purged_offer([3; 32], scope.clone(), key, b"purged".to_vec());
...
mod contract_tests {
    ...
    #[test]
    fn projection_context_uses_offer_value_not_breaking_older_fact_bytes() {
        let store = Db::open_memory_with_schema_sources(&[crate::core::schema::CORE_SCHEMA_SOURCE])
            .expect("open db");
    ...
    #[test]
    fn projection_context_preserves_offer_owner_metadata_and_value() {
        let store = Db::open_memory_with_schema_sources(&[crate::core::schema::CORE_SCHEMA_SOURCE])
            .expect("open db");
        ...
        let projector = test_projector({
            ...
            move |fact, context| {
                if fact.id == producer_id {
                    return Ok(ProjectionOutput::new().offer(ContextOffer::for_key(
                        fact.id,
                        role.clone(),
                        FactScope::Global,
                        key.as_bytes().to_vec(),
                ...
                assert_eq!(matched.offer_owner(), producer_id);
                assert_eq!(&matched.offer_owner_scope, &producer_scope);
                assert_eq!(matched.offer_owner_received_at, 41);
                assert_eq!(matched.value(), value.as_slice());
    ...
    fn projection_drain_resolves_new_need_that_matches_existing_offer() {
        ...
        let projector = need_until_offer_owner(role, key, "ready", Some("premature"));
        let progress =
    ...
    fn projection_drain_uses_context_attached_to_pending_queue() {
        ...
        let projector = need_until_offer_owner(role, key, "queued_context_ready", None);
        let progress =
    ...
    fn ephemeral_input_can_use_existing_durable_context() {
        ...
        let projector = need_until_offer_owner(role.clone(), key.clone(), "ephemeral_ready", None);
        let progress =
    ...
    fn need_until_offer_owner(
        ...
        test_projector(move |fact, context| {
            ...
            if let Some(matched) = context.match_for(&need) {
                Ok(ProjectionOutput::new().intent(Intent::new(
    ...
    fn reemitted_need(role: Role, key: ContextKey, intent_kind: &'static str) -> impl Projector {
        test_projector(move |fact, context| {
            ...
            if context.match_for(&need).is_some() {
                output = output.intent(Intent::new(
    ...
    impl Projector for MultiStageDependencyProjector {
        fn project(
            ...
            let Some(first_match) = context.match_for(&first_need) else {
                return Ok(ProjectionOutput::new().need(first_need));
            };
            let second_need = need_for(fact, &self.second_role, &self.second_key);
            let Some(second_match) = context.match_for(&second_need) else {
                return Ok(ProjectionOutput::new().need(first_need).need(second_need));
            };

            let mut ready_value = first_match.offer_owner().to_vec();
            ready_value.extend_from_slice(&second_match.offer_owner());
            Ok(ProjectionOutput::new().intent(Intent::new(
    ...
    impl Projector for SemanticOfferProjector {
        fn project(
            ...
            if context
                .match_for(&need)
                .is_some_and(|matched| matched.value() != value)
            {
                return Err("matched context did not expose offer value".to_string());
            }
            Ok(ProjectionOutput::new().intent(Intent::new(
...
mod tests {
    ...
    #[test]
    fn projection_context_returns_matches_by_need() {
        let role = Role::new("exact").unwrap();
        ...
        let context = ProjectionContext::from_matches(vec![
            matched_offer_for_need(need_a.clone(), [11; 32]),
            matched_offer_for_need(need_b.clone(), [22; 32]),
            matched_offer_for_need(need_a.clone(), [33; 32]),
        ]);

        let matched_offer_owner_ids = context
            .matches_for(&need_a)
            .map(|matched| matched.offer_owner())
            .collect::<Vec<_>>();
        assert_eq!(matched_offer_owner_ids, vec![[11; 32], [33; 32]]);
        assert_eq!(
            context
                .match_for(&need_b)
                .map(|matched| matched.offer_owner()),
    ...
    fn matched_offer_for_need(need: ContextNeed, offer_owner: FactId) -> MatchedContext {
        MatchedContext {
            offer: ContextOffer {
                owner: offer_owner,
                ...
                value: offer_owner.to_vec(),
            },
            offer_owner_scope: need.scope.clone(),
            offer_owner_received_at: 1,
            need,
        }
    }
