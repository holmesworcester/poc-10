...
///
/// Incoming rows carry this only while they are volatile, so a projector can
/// read it while the row is staged in `incoming_facts`. It is process-local and
/// never copied into durable core storage: once an incoming fact is retained or
/// dropped, the metadata is gone. Origin evidence that must survive replay has
/// to be baked into ordinary protocol fact bytes (for example a
/// `connection_fact_receipt`) emitted by a projector during the incoming run.
    ...
    fn load_pending_inputs(
        ...
        match self {
            ProjectionSource::Durable => {
                ...
                // Durable retained facts never carry ambient incoming origin
                // metadata. Origin evidence survives replay only inside ordinary
                // protocol fact bytes (such as a `connection_fact_receipt`) that
                // reproject on their own, so a durable projection run must not be
                // able to read receive metadata from core.
                let context =
...
pub mod context {
    ...
    impl ProjectionContext {
        ...
        /// Return receive metadata for incoming facts retained from the network.
...
fn move_incoming_to_retained_in_tx(store: &Db, fact: &Fact) -> rusqlite::Result<bool> {
    // Retention copies only the fact bytes and admission metadata. The volatile
    // incoming row's origin address and receive time are intentionally dropped:
    // core keeps no durable receive metadata, so any origin evidence that must
    // outlive the volatile row was already emitted as a protocol fact during the
    // incoming projection run.
...
mod contract_tests {
    ...
    #[test]
    fn retained_incoming_fact_has_no_ambient_origin_metadata_from_core() {
        // Origin address and receive time are volatile: they live on the
        // `incoming_facts` row and are dropped when the fact is retained. A later
        // durable projection run must not be able to read receive metadata from
        // core, because nothing durable in core preserves it. Origin evidence
        // that needs to survive belongs in protocol fact bytes, not here.
        let store = Db::open_memory_with_schema_sources(&[crate::core::schema::CORE_SCHEMA_SOURCE])
            .expect("open db");
