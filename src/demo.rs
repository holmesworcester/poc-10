...
//! target `WakeLoop`, target projectors, and target row tables only. It
//! deliberately does not touch `src/legacy/protocol/` or `src/legacy/workers/`.
//!
//! Run with: `cargo run --example match_demo`
...
use crate::event_modules::sealed_message::create::{associated_data, recover_text, send_message};
...
pub fn run() -> Result<(), String> {
    ...
    println!("------------------------------------------");
    println!("Driving facts through: target WakeLoop -> target projectors -> target row tables.");
    println!("No src/legacy/protocol/ or src/legacy/workers/ code is invoked.");
    ...
    println!("  wake-loop admission for this fact is intentionally skipped here so the demo never");
    println!("  appears to project an envelope through the sealed_message projector.");

    println!("\nresult: target WakeLoop admitted 6 fact types (workspace + signer + message +",);
