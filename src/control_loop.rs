...
pub fn drain_ready(store: &Store, limit: usize) -> Result<ApplyReadyReport, String> {
    store
...
pub fn drain_until_idle(store: &Store, batch_size: usize) -> Result<ApplyReadyReport, String> {
    let mut total = ApplyReadyReport::default();
    loop {
        let report = drain_ready(store, batch_size)?;
        total.applied_events += report.applied_events;
        total.unblocked_events += report.unblocked_events;
