...
//! `crate::legacy::workers::sync` owns scheduling, queue consumption, and worker state.
...
pub mod queries;
pub mod rows;
pub use crate::legacy::workers::sync as worker;
pub use crate::legacy::workers::sync::SyncIndex;
...
fn ensure_record_connection(
    ...
    let actual = match record.scope {
        crate::legacy::protocol::event_modules::types::EventScope::Connection(scope) => {
            scope.connection_id()
        }
