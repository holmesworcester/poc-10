...
use crate::legacy::protocol::event_modules::content::file_deletion::types::{
...
use crate::legacy::protocol::event_modules::content::message_deletion::rows::{
    purge_instruction_row, PurgeKind,
};
use crate::legacy::protocol::event_modules::content::message_deletion::types::deletion_label_author as message_deletion_label_author;
use crate::legacy::protocol::event_modules::identity::{endpoint_shared, signed, user};
use crate::legacy::protocol::event_modules::leaf_history_node;
...
mod tests {
    ...
    use crate::legacy::protocol::event_modules::content::message;
    use crate::legacy::protocol::event_modules::encryption::local_history_node_secret as leaf_module;
    use crate::legacy::protocol::event_modules::identity::{endpoint_shared, signed, user};
    use crate::legacy::protocol::event_modules::types::{event_id, EventScope};
    use crate::legacy::protocol::event_modules::worker::{
    ...
    fn self_deletion_label_suppresses_file_rows_and_emits_purge_intent() {
        ...
        event.context.updates.push(
            crate::legacy::protocol::event_modules::content::file_deletion::types::deletion_label(
                &built.author_id,
            ),
        );
        ...
        assert_eq!(
            ...
            crate::legacy::protocol::event_modules::content::message_deletion::rows::PURGE_INSTRUCTIONS
        );
    ...
    fn deleted_parent_message_label_suppresses_file_and_marks_descriptor_deleted() {
        ...
        event
            ...
            .updates
            .push(
                crate::legacy::protocol::event_modules::content::message_deletion::types::deletion_label(
                    &built.author_id,
                ),
            );
    ...
    fn rejects_parent_message_for_other_workspace() {
        ...
        built.file_event_id = crate::legacy::protocol::event_modules::types::event_id(&bytes);
    ...
    fn rejects_descriptor_whose_leaf_coord_diverges_from_canonical_fields() {
        ...
        let resigned = layout::sign(built.signer_id, &[9; 32], payload);
        let bytes = layout::encode_signed(&resigned);
        let new_id = crate::legacy::protocol::event_modules::types::event_id(&bytes);
    ...
    fn raw_file_bytes_are_not_admissible() {
        ...
        let envelope = layout::decode_signed(&built.record.canonical_bytes).expect("decode signed");
        assert_eq!(
            crate::legacy::protocol::event_modules::event_from_bytes(envelope.payload)
