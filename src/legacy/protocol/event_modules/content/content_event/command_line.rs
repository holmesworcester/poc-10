...
fn run_generate_command(context: &mut Context, args: CliArgs<'_>) -> Result<CliOutput, String> {
    ...
    let membership_key =
        endpoint_shared::rows::endpoint_membership_key(local.endpoint, workspace_id);
    let membership_bytes = context
        .store
        .table_row(endpoint_shared::rows::ENDPOINT_MEMBERSHIPS, &membership_key)
        .map_err(|err| format!("load local endpoint membership: {err}"))?
        .ok_or_else(|| "local endpoint is not joined to workspace".to_string())?;
    let membership =
        endpoint_shared::rows::decode_endpoint_membership_row(&membership_key, &membership_bytes)?;
