...
use crate::legacy::protocol::event_modules::content::{file, message};
...
pub fn admit_check_received(store: &Store, bytes: &[u8]) -> Result<AdmitDecision, String> {
    ...
    let (_slice, file_event_id) = layout::decode(&envelope.payload)?;
    let descriptor_bytes =
        crate::legacy::protocol::event_modules::rows::event_bytes(store, &file_event_id)
    ...
    let descriptor_envelope = file::layout::decode_signed(&descriptor_bytes)?;
    let descriptor = file::layout::decode(&descriptor_envelope.payload)?;
