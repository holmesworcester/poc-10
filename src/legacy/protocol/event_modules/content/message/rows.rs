...
use crate::legacy::protocol::event_modules::encryption::local_history_node_secret::queries as local_history_queries;
use crate::legacy::protocol::event_modules::types::EventId;
use crate::legacy::protocol::event_modules::worker::AdmitDecision;
...
pub fn admit_check_received(store: &Store, bytes: &[u8]) -> Result<AdmitDecision, String> {
    let envelope = layout::decode_signed(bytes)?;
    let event = layout::decode(&envelope.payload)?;
    let event_id = crate::legacy::protocol::event_modules::types::event_id(bytes);
...
pub fn decode_message_row(key: &[u8], value: &[u8]) -> Result<MessageRow, String> {
    ...
    let author_user_id = reader.id()?;
    let signer_endpoint_shared_id = reader.id()?;
    let text = layout::decode_text_slot(reader.slice(MESSAGE_TEXT_BYTES)?)?;
    reader.finish()?;
...
fn encode_value(
    ...
    let text = layout::encode_text_slot(&event.text)?;
...
mod tests {
    ...
    fn signed_message_bytes(
        ...
        let envelope = layout::sign([8; 32], &[7; 32], payload);
        layout::encode_signed(&envelope)
    }
    ...
    fn seed_frontier_root(store: &Store, workspace_id: EventId) {
        let row = local_key_secret::rows::local_key_secret_row(
    ...
    fn admit_drops_message_with_existing_tombstone_silently() {
        ...
        assert!(
            crate::legacy::protocol::event_modules::rows::event_bytes(&store, &id)
                .expect("event bytes")
                .is_none(),
