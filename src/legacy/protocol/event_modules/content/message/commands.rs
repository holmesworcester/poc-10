...
pub fn send(input: SendMessage) -> Result<CommandOutput<SendMessageOutput>, String> {
    ...
    let value = SendMessageOutput {
        message_id: crate::legacy::protocol::event_modules::types::event_id(
...
pub fn require_local_membership(
    ...
    crate::legacy::protocol::event_modules::identity::endpoint_shared::types::EndpointMembershipRow,
    String,
> {
...
pub fn next_authoring_timestamp(store: &Store, workspace_id: EventId) -> Result<u64, String> {
    ...
    let from_content =
        crate::legacy::protocol::event_modules::content::content_event::queries::max_timestamp_for_workspace(
            store,
            workspace_id,
        )?;
...
pub fn is_deleted_by_author(
    ...
    let updates = crate::legacy::protocol::event_modules::rows::context_updates(store, message_id)
        .map_err(|err| format!("load deletion updates: {err}"))?;
    Ok(updates.iter().any(|update| {
        deletion_label_author(update)
