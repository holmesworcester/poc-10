...
use crate::legacy::protocol::event_modules::content::reaction;
use crate::legacy::protocol::event_modules::identity::{endpoint, user};
...
use super::{commands, queries, rows};
...
fn sealed_message_rows_for_workspace(
    store: &Store,
    workspace_id: EventId,
) -> Result<Vec<rows::SealedMessageRow>, String> {
    store
        .table_rows_with_key_prefix(rows::SEALED_MESSAGES, &workspace_id, usize::MAX)
        .map_err(|err| format!("load sealed messages: {err}"))?
        .into_iter()
        .map(|(key, value)| rows::decode_sealed_message_row(&key, &value))
        .collect()
}
...
fn sealed_reaction_rows_for_workspace(
    store: &Store,
    workspace_id: EventId,
) -> Result<Vec<reaction::rows::SealedReactionRow>, String> {
    store
        .table_rows_with_key_prefix(reaction::rows::SEALED_REACTIONS, &workspace_id, usize::MAX)
        .map_err(|err| format!("load sealed reactions: {err}"))?
        .into_iter()
        .map(|(key, value)| reaction::rows::decode_sealed_reaction_row(&key, &value))
        .collect()
}
...
where
    R: crate::legacy::workers::pipeline_helpers::event_pipeline::EventRegistry,
{
...
fn user_name(store: &Store, workspace_id: EventId, user_id: EventId) -> Result<String, String> {
    ...
    match value {
        Some(value) => {
            let row = user::rows::decode_user_row(&key, &value)?;
            Ok(row.username)
        }
