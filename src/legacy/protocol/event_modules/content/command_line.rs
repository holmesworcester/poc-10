...
use crate::legacy::protocol::event_modules::content::{file, file_slice, message};
...
use crate::legacy::protocol::event_modules::worker::{self, CommandOutput, ProposedEvent};
...
fn run_view_command(context: &mut Context, args: CliArgs<'_>) -> Result<CliOutput, String> {
    ...
    let local = endpoint::commands::local_keypair(&context.store)?
        .ok_or_else(|| "local endpoint is missing".to_string())?;
    let joined = endpoint_shared::rows::workspace_ids_for_endpoint(&context.store, local.endpoint)?;
    ...
    let workspace_row = workspace::rows::decode_workspace_row(&workspace_id, &workspace_value)?;
...
fn workspace_user_view(
    ...
    for (key, value) in store
        .table_rows_with_key_prefix(user::rows::USERS, &workspace_id, usize::MAX)
    ...
    for (key, value) in store
        .table_rows_with_key_prefix(
            endpoint_shared::rows::ENDPOINT_SHARED,
            &workspace_id,
            usize::MAX,
        )
        .map_err(|err| format!("load endpoint_shared: {err}"))?
    {
        let row = endpoint_shared::rows::decode_endpoint_shared_row(&key, &value)?;
