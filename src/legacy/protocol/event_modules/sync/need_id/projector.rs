...
mod tests {
    ...
    fn outgoing_need_id_projects_cached_event_and_transit_out_rows() {
        let record = layout::outbound_record(need_event()).expect("record");
    ...
    fn incoming_need_id_projects_inbound_sync_work_row() {
        let bytes = layout::encode(&need_event());
    ...
    fn need_id_rejects_connection_scope_mismatch() {
        let mut record = layout::outbound_record(need_event()).expect("record");
