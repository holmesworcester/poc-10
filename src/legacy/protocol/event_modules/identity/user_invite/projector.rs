...
use crate::legacy::protocol::event_modules::identity::{admin, endpoint_shared, signed, workspace};
...
pub fn project(event: &EventWithContext<'_>) -> Result<ProjectionOutput, String> {
    ...
    let user_invite = layout::decode(&envelope.payload)?;
    let signer = event
        .context
...
#[cfg(test)]
mod tests {
    use crate::legacy::protocol::event_modules::identity::workspace::types::WorkspaceEvent;
    use crate::legacy::protocol::event_modules::types::{event_id, EventId, EventRecord};
    use crate::legacy::protocol::event_modules::worker::{DependencyContext, EventContext};
    ...
    fn workspace_record(
        ...
        crate::legacy::protocol::event_modules::types::EventId,
        EventRecord,
    ) {
    ...
    fn admin_record(workspace_id: EventId, admin_user_id: EventId) -> (EventId, EventRecord) {
        ...
        let bytes = admin::layout::encode(&admin);
        let admin_id = event_id(&bytes);
        (
            admin_id,
            admin::layout::record_from_bytes(bytes).expect("admin record"),
        )
    }
    ...
    fn endpoint_shared_record(
        ...
        let signed = signed::commands::sign_payload(
            [44; 32],
            &[45; 32],
            endpoint_shared::layout::encode(&endpoint_shared).expect("encode endpoint_shared"),
        )
    ...
    fn projects_workspace_signed_user_invite_row() {
        ...
        assert_eq!(
            ...
            rows::user_invite_key(&workspace_id, &event_id(&record.canonical_bytes))
        );
        assert_eq!(
            rows::decode_user_invite_row(
                &output.legacy_rows()[0].key,
    ...
    fn projects_admin_signed_user_invite_from_endpoint_owned_by_admin_user() {
        ...
        assert_eq!(output.legacy_rows().len(), 1);
        let row = rows::decode_user_invite_row(
            &output.legacy_rows()[0].key,
            &output.legacy_rows()[0].value,
        )
        .expect("decode row");
