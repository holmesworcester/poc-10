...
use crate::legacy::protocol::event_modules::types::{EventId, EventRecord, EventScope};
use crate::legacy::protocol::wire::{Reader, Writer};
...
fn inner_metadata(event: &SignedEnvelope) -> Result<InnerMetadata, String> {
    ...
    let (timestamp, body_len, inner_dependencies, workspace_id) = match event.inner_type {
        user_invite::layout::TYPE_USER_INVITE => {
            let inner = user_invite::layout::decode(&event.payload)?;
            (
                inner.created_at_ms,
                user_invite::layout::USER_INVITE_WIRE_SIZE - 1,
                vec![inner.workspace_id, inner.authority_event_id],
                Some(inner.workspace_id),
            )
        }
        invite_server::layout::TYPE_INVITE_SERVER => {
            let inner = invite_server::layout::decode(&event.payload)?;
            (
                inner.created_at_ms,
                invite_server::layout::INVITE_SERVER_WIRE_SIZE - 1,
                vec![inner.workspace_id, inner.authority_event_id],
                Some(inner.workspace_id),
            )
        }
        user::layout::TYPE_USER => {
            let inner = user::layout::decode(&event.payload)?;
            (
                inner.created_at_ms,
                user::layout::USER_WIRE_SIZE - 1,
                vec![inner.workspace_id],
                Some(inner.workspace_id),
            )
        }
        device_invite::layout::TYPE_DEVICE_INVITE => {
            let inner = device_invite::layout::decode(&event.payload)?;
            (
                inner.created_at_ms,
                device_invite::layout::DEVICE_INVITE_WIRE_SIZE - 1,
                device_invite::layout::dependencies(&inner),
                Some(inner.workspace_id),
            )
        }
        endpoint_shared::layout::TYPE_ENDPOINT_SHARED => {
            let inner = endpoint_shared::layout::decode(&event.payload)?;
            (
                inner.created_at_ms,
                endpoint_shared::layout::ENDPOINT_SHARED_WIRE_SIZE - 1,
                vec![inner.workspace_id, inner.user_authority_event_id],
                Some(inner.workspace_id),
            )
        }
        admin::layout::TYPE_ADMIN => {
            let inner = admin::layout::decode(&event.payload)?;
            (
                inner.created_at_ms,
                admin::layout::ADMIN_WIRE_SIZE - 1,
                admin::layout::dependencies(&inner),
                Some(inner.workspace_id),
            )
        }
...
mod tests {
    ...
    use crate::legacy::protocol::event_modules::identity::{admin, endpoint_shared, user_invite};
    ...
    fn signed_user_invite_uses_inner_timestamp_and_unique_dependencies() {
        let payload = user_invite::layout::encode(&user_invite::types::UserInviteEvent {
            created_at_ms: 1234,
            public_key: [4; 32],
            workspace_id: [1; 32],
    ...
    fn signed_admin_uses_inner_timestamp_and_unique_dependencies() {
        ...
        assert_eq!(record.body_len, admin::layout::ADMIN_WIRE_SIZE - 1);
        assert_eq!(
            record.dependencies,
