...
use crate::legacy::protocol::event_modules::connection::connection_request;
use crate::legacy::protocol::event_modules::queries as event_queries;
use crate::legacy::protocol::event_modules::types::EventId;
use crate::legacy::protocol::event_modules::worker::{self, CommandOutput};
use crate::legacy::workers::{connection as connection_worker, transit_in, DaemonWorkerContext};
...
fn run_workspaces_command(context: &mut Context, args: CliArgs<'_>) -> Result<CliOutput, String> {
    ...
    for workspace_id in
        endpoint_shared::rows::workspace_ids_for_endpoint(&context.store, local.endpoint)?
    {
...
fn connect_invite(context: &mut Context, link: &str) -> Result<ConnectReport, String> {
    ...
    let from_listen_addr = crate::legacy::daemon::current_listen_addr(&context.db_path)?;
    let output =
...
fn reject_duplicate_join(
    ...
    if store
        .table_row(
            endpoint_shared::rows::ENDPOINT_MEMBERSHIPS,
            &endpoint_shared::rows::endpoint_membership_key(endpoint_id, workspace_id),
        )
    ...
    if !store
        .table_rows_with_key_prefix(invite_accepted::rows::INVITES_ACCEPTED, &accepted_prefix, 1)
...
fn workspace_row(
    ...
    let value = store
        .table_row(workspace::rows::WORKSPACES, &workspace_id)
        .map_err(|err| format!("load workspace: {err}"))?
        .ok_or_else(|| "workspace row is missing".to_string())?;
    workspace::rows::decode_workspace_row(&workspace_id, &value)
}
