...
use crate::legacy::protocol::event_modules::identity::{endpoint, endpoint_shared, invite, user};
...
fn user_row(
    ...
    user::rows::decode_user_row(&key, &value)
}
...
fn admin_for_user(
    ...
    for (key, value) in store
        .table_rows_with_key_prefix(super::rows::ADMINS, &workspace_id, usize::MAX)
        .map_err(|err| format!("load admins: {err}"))?
    {
        let row = super::rows::decode_admin_row(&key, &value)?;
