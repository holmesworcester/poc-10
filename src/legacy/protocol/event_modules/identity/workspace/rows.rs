...
pub const SCHEMAS: &[Schema] = &[Schema::durable_row_table(
    "identity.workspaces.v3",
    WORKSPACES,
)];

const WORKSPACE_VALUE_BYTES: usize = 8 + 32 + WORKSPACE_NAME_BYTES;
...
fn encode_workspace_value(
    ...
    let encoded = layout::encode(&event)?;
    let mut reader = Reader::new(&encoded, "workspace value source");
    let tag = reader.u8()?;
    if tag != layout::TYPE_WORKSPACE {
...
fn workspace_value_name(bytes: &[u8]) -> Result<String, String> {
    let encoded = {
        let mut out = Writer::with_capacity(layout::WORKSPACE_WIRE_SIZE);
        out.u8(layout::TYPE_WORKSPACE);
        out.u64(0);
        out.id(&[0; 32]);
        out.raw(bytes);
        out.finish()
    };
    layout::decode(&encoded).map(|event| event.name)
}
