...
use crate::legacy::protocol::event_modules::identity::endpoint::types::{EndpointId, EndpointRole};
...
pub fn decode_endpoint_shared_row(key: &[u8], value: &[u8]) -> Result<EndpointSharedRow, String> {
    ...
    let device_name =
        layout::decode_device_name(reader.slice(super::types::ENDPOINT_DEVICE_NAME_BYTES)?)?;
    reader.finish()?;
...
fn encode_endpoint_shared_value(
    ...
    let device_name = layout::encode_device_name(&event.device_name)?;
    let mut out =
