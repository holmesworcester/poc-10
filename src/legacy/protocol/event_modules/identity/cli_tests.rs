...
fn unknown_signed_identity_payloads_are_rejected_at_admission() {
    ...
    let err = crate::legacy::protocol::event_modules::event_from_bytes(
...
fn run_protocol_cli(db: &std::path::Path, args: &[&str]) -> String {
    let mut context =
        crate::legacy::protocol::commands::Context::open(db).expect("open cli context");
    let command_args = args.iter().map(|arg| arg.to_string()).collect::<Vec<_>>();
    let output = crate::core::commands::run(
        &crate::legacy::protocol::commands::commands(),
        &mut context,
        &command_args,
    )
...
fn unsigned_device_invite_bytes_are_not_admissible_protocol_events() {
    ...
    let err = crate::legacy::protocol::event_modules::event_from_bytes(
        device_invite::layout::encode(&raw),
    )
