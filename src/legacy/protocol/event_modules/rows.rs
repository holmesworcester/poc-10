//! Protocol-wide rows and row helpers for the common event pipeline.
//!
...
//! `rows.rs`; do not push it down into `core::store`.
//!
...
//! event is. Operational worker queues live under `src/legacy/workers/rows.rs`.
...
// `queries::` in `projector.rs` and sibling rows files import other
// rows modules' queries via this stable path).
