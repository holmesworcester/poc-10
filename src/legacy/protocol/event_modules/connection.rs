...
//! live under `src/legacy/workers` and bridge opaque network frames to canonical event
...
pub fn is_projection_record(bytes: &[u8]) -> bool {
    connection_ephemeral_secret::layout::is_ephemeral_secret(bytes)
        || connection_request::layout::is_request(bytes)
        || connection_response::layout::is_response(bytes)
}
...
pub fn project_record(event: &EventWithContext<'_>) -> Result<ProjectionOutput, String> {
    ...
    if connection_ephemeral_secret::layout::is_ephemeral_secret(bytes) {
...
pub fn event_from_bytes(bytes: Vec<u8>) -> Result<EventRecord, String> {
    if connection_ephemeral_secret::layout::is_ephemeral_secret(&bytes) {
        return connection_ephemeral_secret::layout::record_from_bytes(bytes);
    }
    if connection_request::layout::is_request(&bytes) {
        return connection_request::layout::record_from_bytes(bytes);
    }
    if connection_response::layout::is_response(&bytes) {
        return connection_response::layout::record_from_bytes(bytes);
    }
