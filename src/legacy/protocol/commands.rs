...
use crate::legacy::protocol::{event_modules, Protocol};
...
impl EventRegistry for Context {
    ...
    fn record_from_canonical_in(
        ...
        provenance: Option<crate::legacy::workers::queue_rows::TransitProvenance>,
...
fn run_sync_status_command(context: &mut Context, args: CliArgs<'_>) -> Result<CliOutput, String> {
    ...
    let _ = crate::legacy::workers::sync::run(
        &context.store,
        index,
        crate::legacy::workers::sync::Work::DrainPendingPurges { limit: usize::MAX },
    )
...
fn run_negentropy_drain_command(
    ...
    let limit = match args.get(0) {
        ...
        None => crate::legacy::workers::sync::DEFAULT_PURGE_DRAIN_LIMIT,
    };
    let index = context.sync_index();
    ...
    index
        .catch_up(&context.store)
        .map_err(|err| format!("catch up sync index: {err}"))?;
    let output = crate::legacy::workers::sync::run(
        &context.store,
        index,
        crate::legacy::workers::sync::Work::DrainPendingPurges { limit },
    )
    .map_err(|err| format!("drain negentropy purges: {err}"))?;
    let report = match output {
        crate::legacy::workers::sync::Output::DrainedPurges(report) => report,
