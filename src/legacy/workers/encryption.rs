...
use crate::legacy::protocol::event_modules::identity::{endpoint, endpoint_shared};
use crate::legacy::protocol::event_modules::queries as event_queries;
use crate::legacy::protocol::event_modules::rows as event_schema;
use crate::legacy::protocol::event_modules::types::{event_id, EventId};
use crate::legacy::protocol::event_modules::worker::{self, EventRegistry};
use crate::legacy::workers::{dependency_unblock, queue_rows as worker_rows};
use crate::legacy::workers::{event_lifecycle, event_retention};
    ...
    store
        .write_transaction(|tx| {
            let inserted_event = event_lifecycle::insert_event(
                tx,
                &record,
                crate::legacy::protocol::event_modules::types::EventStatus::Applied,
            )?;
...
fn gc_subsumed_tombstones(
    ...
    let message_keys_to_delete: Vec<Vec<u8>> =
        crate::legacy::protocol::event_modules::content::message::queries::list_message_tombstones_for_workspace(
            store,
            workspace_id,
        )?
        .into_iter()
        ...
        .map(|row| {
            crate::legacy::protocol::event_modules::content::message::rows::message_key(
                row.workspace_id,
                row.message_id,
            )
        })
    ...
    let message_table =
        crate::legacy::protocol::event_modules::content::message::rows::MESSAGE_TOMBSTONES;
...
fn drain_pending_message_leaves<R: EventRegistry>(
    ...
    use crate::legacy::protocol::event_modules::content::{file, message, reaction};
...
pub(crate) fn daemon_worker<C>() -> crate::legacy::daemon::Worker<C>
where
    C: crate::legacy::workers::DaemonWorkerContext,
{
    use crate::legacy::daemon::{StepContext, Worker};
...
mod tests {
    ...
    use crate::legacy::protocol::event_modules::identity::{
        endpoint, endpoint_shared, signed, workspace,
    };
    use crate::legacy::protocol::event_modules::types::{event_id, EventStatus};
    ...
    fn seed_workspace_dependency(store: &Store) {
        ...
        store
            .insert_table_rows(vec![
                crate::legacy::protocol::event_modules::rows::event_row(
                    &WORKSPACE,
                    &record,
                    EventStatus::Applied,
                )
