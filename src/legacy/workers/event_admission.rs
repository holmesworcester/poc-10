...
use crate::legacy::workers::pipeline_helpers::event_pipeline::{
    ...
    C: DaemonWorkerContext,
{
    daemon::run_step(
        ...
        |report, daemon_report| {
            daemon_report.add("admitted_events", report.inserted_events);
            daemon_report.add("blocked_events", report.blocked_events);
            daemon_report.add("applied_events", report.applied_events);
        },
    )
}
...
mod tests {
    use crate::core::store::Store;
    use crate::legacy::protocol::event_modules::connection::{self, types};
    use crate::legacy::protocol::event_modules::content::content_event;
