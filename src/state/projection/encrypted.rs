...
pub fn project_encrypted(
    ...
    // plan.md Stage 2: PK is (workspace_id, event_id); the row is unique
    // per workspace so we no longer filter by recorded_by — sibling
    // tenants share the projection.
    let _ = recorded_by;
    let key_bytes: Vec<u8> = match conn.query_row(
        "SELECT key_bytes FROM key_secrets WHERE event_id = ?1 LIMIT 1",
        rusqlite::params![event_id_to_base64(&enc.key_event_id)],
        |row| row.get(0),
    ) {
        Ok(k) => k,
