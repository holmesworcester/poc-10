...
pub(crate) fn apply_projection(
    ...
    apply_projection_with_backend(conn, recorded_by, event_id_b64, blob, parsed)
}
...
pub(crate) fn apply_projection_with_backend<B: ProjectionBackend>(
    ...
    // Build projector context via the generic loader.
    //
    // Per plan.md (commit afc171015718e9a1e), the strict context contract is
    // `{event, deps, labels}` and nothing else. The legacy per-event
    // `EventTypeMeta::context_loader` callback has been retired; the
    // substrate fills `ctx.deps` + `ctx.labels` (+ sync-specific fields)
    // and projectors that need richer state must declare it as a dep or
    // a label.
    let mut ctx = backend.load_generic_context(recorded_by, parsed)?;

    // The generic loader's `labels` lookup is bounded to `own_id ++ deps`.
    // The dispatcher additionally surfaces labels keyed by payload-shaped
    // identity refs (signer / author / target) so message/reaction/deletion
    // projectors can consult `removed_by:*` / `deleted` gates without
    // re-issuing SQL of their own. We merge those in on top of whatever
    // the generic loader already produced.
    {
        use std::collections::BTreeSet;
        let mut id_set: BTreeSet<crate::crypto::EventId> = BTreeSet::new();
        match parsed {
            ParsedEvent::MessageDeletion(d) => {
                id_set.insert(d.target_event_id);
                id_set.insert(d.author_id);
                id_set.insert(d.signed_by);
            }
            ParsedEvent::Reaction(r) => {
                id_set.insert(r.target_event_id);
                id_set.insert(r.author_id);
                id_set.insert(r.signed_by);
            }
            ParsedEvent::Message(m) => {
                id_set.insert(m.author_id);
                id_set.insert(m.signed_by);
            }
            _ => {}
        }
        if !id_set.is_empty() {
            let id_vec: Vec<crate::crypto::EventId> = id_set.into_iter().collect();
            let extra = backend.load_labels(recorded_by, &id_vec)?;
            for (k, v) in extra {
                ctx.labels.entry(k).or_insert(v);
            }
        }
    }
    // Dispatch to pure projector. The legacy path threads `recorded_by`
    // through a scoped thread-local so projectors can honour the per-
    // tenant scope while the projector signature itself (Stage 2 of the
    // `recorded_by` retirement, plan.md) is parameter-free. The new
    // inbound chain leaves the slot empty and projectors fall back to
    // the chain sentinel.
    let result = super::dispatch::with_recorded_by(recorded_by, || {
        dispatch_pure_projector(event_id_b64, parsed, &ctx)
    });
...
mod tests {
    ...
    use crate::event_modules::KeySecretEvent;
