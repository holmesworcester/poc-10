    ...
    for cmd in commands {
        match cmd {
            ...
            EmitCommand::MaterializeTransportIdentity { spec } => {
                ...
                let materializer =
                    crate::runtime::legacy_identity::ConcreteTransportIdentityMaterializer;
...
mod tests {
    ...
    fn insert_invite_secret(
        ...
        // Plan.md Stage 3.5 step 5B: `invite_secrets.recorded_by` dropped.
        // The `recorded_by` parameter doubles as a workspace_id for tests
        // that don't care about a real workspace_id.
        ...
        conn.execute(
            "INSERT INTO invite_secrets
             (workspace_id, event_id, invite_event_id, private_key, created_at)
             VALUES (?1, ?2, ?3, ?4, 0)",
    ...
    fn execute_emit_commands_records_transport_target_for_peershared_intent() {
        ...
        let workspace_id_b64 = crate::crypto::event_id_to_base64(&[0x77u8; 32]);
        ...
        // secret_event_id is UNIQUE, signer_event_id is the signer index.
        conn.execute(
