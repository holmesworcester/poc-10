...
fn test_invite_accepted_requires_tenant_not_workspace() {
    ...
    let anchor: String = conn
        .query_row(
            "SELECT workspace_id FROM invites_accepted ORDER BY created_at ASC, event_id ASC LIMIT 1",
            [],
            |row| row.get(0),
        )
        .unwrap();
    ...
    let ia_count: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM invites_accepted WHERE event_id = ?1",
            rusqlite::params![&ia_b64],
            |row| row.get(0),
        )
        .unwrap();
...
fn test_full_bootstrap_progression_from_projected_sql_state() {
    ...
    let ps_spki = crate::crypto::spki_fingerprint_from_ed25519_pubkey(&peer_shared_pub);
    let ps_allowed =
...
fn test_bootstrap_trust_superseded_by_matching_peer_shared() {
    ...
    let peer_shared_pub = peer_shared_key.verifying_key().to_bytes();
    let bootstrap_spki =
        crate::crypto::spki_fingerprint_from_ed25519_pubkey(&peer_shared_pub);
...
fn test_invite_accepted_same_workspace_binding_is_scoped_per_tenant() {
    ...
    // Plan.md Stage 3.5 step 5B: invites_accepted.recorded_by dropped.
    // Two tenants accepting invites to the same workspace now produce two
    // distinct rows keyed by (workspace_id, event_id) — the per-tenant
    // scoping is captured by the event_id (which is per-event), not by a
    // recorded_by column.
    let total_count: i64 = conn
        .query_row(
            "SELECT COUNT(*) FROM invites_accepted WHERE workspace_id = ?1",
            rusqlite::params![&shared_workspace_b64],
            |row| row.get(0),
        )
        .unwrap();
    ...
    assert_eq!(
        total_count, 2,
        "both tenants' workspace bindings should coexist"
    );
}
