...
fn setup_workspace_anchor(conn: &Connection, recorded_by: &str) -> (EventId, SigningKey) {
    ...
    let workspace_event = ParsedEvent::Workspace(WorkspaceEvent {
        ...
        workspace_id: workspace_key.verifying_key().to_bytes(),
...
// Wave 1: admin event dropped. Stub returns user_event_id so callers still
// link; tests that exercised the admin projector path are gated below.
fn project_valid_admin_for_user(
    _conn: &Connection,
    _recorded_by: &str,
    _workspace_eid: EventId,
    _workspace_key: &SigningKey,
    user_event_id: EventId,
    _user_public_key: [u8; 32],
) -> EventId {
    user_event_id
}
...
// Wave 1: admin event dropped; signer-family rejection test removed.
// Wave 1 cleanup: `test_admin_projects_with_workspace_signer_family` was
// deleted because the admin event type has been removed.
// Wave 1: admin public_key mismatch rejection test removed along with the
// admin event module.
...
// Wave 1 cleanup: `test_user_invite_rejects_peer_signed_authority_mismatch_at_projection`
// was deleted because the admin event type has been removed; peer-signed
// `user_invite` paths whose authority resolves through an admin event no
// longer exist.
...
fn test_emit_cross_tenant_records_and_projects() {
    ...
    // key_secrets: 1 row globally (plan.md Stage 2: PK is now
    // (workspace_id, event_id), so cross-tenant inserts dedupe).
    ...
    assert_eq!(
        sk_count, 1,
        "key_secret row dedupes by (workspace_id, event_id) — one global row"
    );
}
