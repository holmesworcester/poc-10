...
fn test_removal_projects_and_rejects_bad_frontier_hash() {
    ...
    // Plan.md round-9 step 1: `removals` PK migrated to
    // `(workspace_id, event_id)`. Resolve the caller's workspace_id
    // through `invites_accepted` (populated by the identity-chain
    // fixture) and assert against the workspace-scoped row.
    // Plan.md Stage 3.5 step 5B: invites_accepted.recorded_by dropped.
    let _ = recorded_by;
    let workspace_id_b64: String = conn
        .query_row(
            "SELECT workspace_id
        ...
        .expect("invites_accepted workspace_id");
    let projected: bool = conn
        .query_row(
            "SELECT COUNT(*) > 0 FROM removals WHERE workspace_id = ?1 AND event_id = ?2",
            rusqlite::params![&workspace_id_b64, &valid_b64],
            |row| row.get(0),
        )
        .unwrap();
...
fn test_key_rotation_projects_root_frontier_and_rejects_bad_hash() {
    ...
    let valid_b64 = event_id_to_base64(&valid_eid);
    // Plan.md round-9 step 2: `key_rotations.recorded_by` shadow column
    // dropped — the row is keyed by `(workspace_id, event_id)`. The test
    // fixture builds rotations with `workspace_id: [0u8; 32]`, so the
    // projector writes that all-zeros workspace_id; assert against it.
    let test_workspace_id_b64 = event_id_to_base64(&[0u8; 32]);
    let projected: bool = conn
        .query_row(
            "SELECT COUNT(*) > 0 FROM key_rotations WHERE workspace_id = ?1 AND event_id = ?2",
            rusqlite::params![&test_workspace_id_b64, &valid_b64],
            |row| row.get(0),
        )
        .unwrap();
...
fn test_key_rotation_blocks_on_missing_frontier_then_projects_and_rejects_bad_hash() {
    ...
    // Plan.md round-9 step 2: workspace-scoped `key_rotations` PK; the
    // test fixture writes rotations with `workspace_id: [0u8; 32]`.
    let test_workspace_id_b64 = event_id_to_base64(&[0u8; 32]);
    let projected: bool = conn
        .query_row(
            "SELECT COUNT(*) > 0 FROM key_rotations WHERE workspace_id = ?1 AND event_id = ?2",
            rusqlite::params![&test_workspace_id_b64, &blocked_b64],
            |row| row.get(0),
        )
        .unwrap();
    assert!(
...
fn test_concurrent_multi_parent_frontier_hash_converges_despite_parent_order() {
    ...
    // Plan.md round-9 step 1: workspace-scoped `removals` PK.
    // Plan.md Stage 3.5 step 5B: invites_accepted.recorded_by dropped.
    let _ = recorded_by;
    let workspace_id_b64: String = conn
        .query_row(
    ...
    let stored_left_right: String = conn
        .query_row(
            "SELECT frontier_hash FROM removals WHERE workspace_id = ?1 AND event_id = ?2",
            rusqlite::params![&workspace_id_b64, &merge_left_right_b64],
            |row| row.get(0),
        )
        .unwrap();
    let stored_right_left: String = conn
        .query_row(
            "SELECT frontier_hash FROM removals WHERE workspace_id = ?1 AND event_id = ?2",
            rusqlite::params![&workspace_id_b64, &merge_right_left_b64],
            |row| row.get(0),
        )
        .unwrap();
...
fn test_key_rotation_multi_parent_frontier_blocks_until_all_frontier_deps_arrive() {
    ...
    let projected: bool = conn
        .query_row(
            "SELECT COUNT(*) > 0 FROM key_rotations WHERE workspace_id = ?1 AND event_id = ?2",
            rusqlite::params![&test_workspace_id_b64, &rotation_b64],
            |row| row.get(0),
        )
        .unwrap();
    assert!(
    ...
    let stored_frontier_hash: String = conn
        .query_row(
            "SELECT frontier_hash FROM key_rotations WHERE workspace_id = ?1 AND event_id = ?2",
            rusqlite::params![&test_workspace_id_b64, &rotation_b64],
            |row| row.get(0),
        )
        .unwrap();
