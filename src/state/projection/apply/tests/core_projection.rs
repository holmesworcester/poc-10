...
fn key_shared_does_not_block_non_recipient_observers_on_local_invite_secret() {
    ...
    let recipient_event_id: EventId = conn
        .query_row(
            // Plan.md Stage 3.5 step 5B: user_invites.recorded_by dropped.
            "SELECT event_id FROM user_invites LIMIT 1",
            [],
            |row| row.get(0),
        )
    ...
    // Plan.md Stage 3.5 step 5C: `key_shared.recorded_by` shadow column
    // dropped — read by `(workspace_id, event_id)`. The test fixture
    // builds key_shared with `workspace_id: [0u8; 32]`.
    let test_workspace_id_b64 = event_id_to_base64(&[0u8; 32]);
    let projected: bool = conn
        .query_row(
            "SELECT COUNT(*) > 0 FROM key_shared WHERE workspace_id = ?1 AND event_id = ?2",
            rusqlite::params![&test_workspace_id_b64, &key_shared_b64],
            |row| row.get(0),
        )
        .unwrap();
    assert!(
    ...
    // Plan.md Stage 3.5 step 5C: `key_secrets.recorded_by` shadow column
    // dropped — count rows scoped to the fixture's all-zeros workspace_id.
    let emitted_key_secret: bool = conn
        .query_row(
            "SELECT COUNT(*) > 0 FROM key_secrets WHERE workspace_id = ?1",
            rusqlite::params![&test_workspace_id_b64],
            |row| row.get(0),
        )
        .unwrap();
    assert!(
...
fn key_shared_blocks_on_missing_frontier_then_projects() {
    ...
    // Plan.md Stage 3.5 step 5C: `key_shared.recorded_by` shadow column
    // dropped — read by `(workspace_id, event_id)`.
    let test_workspace_id_b64 = event_id_to_base64(&[0u8; 32]);
    let key_shared_b64 = event_id_to_base64(&key_shared_eid);
    let projected: bool = conn
        .query_row(
...
fn test_project_key_request_valid_with_delivery_target_binding() {
    ...
    // Plan.md Stage 3.5 step 5C: `key_requests.recorded_by` shadow column
    // dropped — read by `(workspace_id, event_id)`.
    let test_workspace_id_b64 = event_id_to_base64(&[0u8; 32]);
    let key_request_b64 = event_id_to_base64(&key_request_eid);
    let projected: bool = conn
        .query_row(
            "SELECT COUNT(*) > 0 FROM key_requests WHERE workspace_id = ?1 AND event_id = ?2",
            rusqlite::params![&test_workspace_id_b64, &key_request_b64],
            |row| row.get(0),
        )
        .unwrap();
...
// Wave 1: BenchDep duplicate-dep regression test removed along with the
// bench_dep event module (BenchDep was the only event type whose `dep_ids`
// could be a free-form vector with duplicates).
