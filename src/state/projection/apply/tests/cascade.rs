...
// Wave 1: BenchDep multi-dep cascade test removed along with the bench_dep
// event module. The remaining cascade tests in this file exercise the same
// code paths via Reaction-on-Message dependencies.
...
fn test_invite_accepted_guard_retry_on_workspace() {
    ...
    // Plan.md Stage 2: workspace projector now applies cleanly without
    // an accepted-invite anchor (workspace_id is read directly off the
    // event). The legacy guard-block pattern is gone; the
    // RetryWorkspaceEvent command emitted by invite_accepted is now a
    // no-op for the projector itself but still drives the legacy
    // cascade machinery to mark the workspace valid for this tenant.
    let r1 = project_one(&conn, recorded_by, &ws_eid).unwrap();
    assert_eq!(
        r1,
        ProjectionDecision::Valid,
        "workspace projects Valid directly under the new chain (no anchor required)"
    );

    // InviteAccepted still projects Valid and emits its retry command;
    // the workspace was already Valid above, so this is idempotent.
    ...
    // Workspace remains valid (idempotent under the new chain).
    ...
    let ws_valid: bool = conn
        ...
        .unwrap();
    assert!(ws_valid, "workspace should be valid");
}

// Wave 1: file_slice guard-retry cascade test removed along with the file
// and file_slice event modules.
