...
fn store_blob_only(
    ...
    // Local creates always insert a new recorded_events row, so always emit a live hint
    // for shared events.
    let live_hints = if meta.share_scope == crate::event_modules::registry::ShareScope::Shared {
        vec![LiveHintEvent {
...
pub fn create_encrypted_event_synchronous(
    ...
    // plan.md Stage 2: PK is (workspace_id, event_id); read by event_id
    // alone since the row is workspace-scoped.
    let _ = recorded_by;
...
mod tests {
    ...
    // Wave 1 cleanup: `test_local_shared_create_fanout_is_same_workspace_only`
    // was deleted because it relied on multi-tenant-per-workspace fanout
    // within a single shared DB — forbidden by the
    // endpoint_workspace_uniqueness invariant. Per-tenant `recorded_by`
    // partitioning of the same shared event is no longer the model.
}
