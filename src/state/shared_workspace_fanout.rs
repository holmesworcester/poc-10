...
fn sibling_tenants_in_workspace(
    ...
    // The legacy fanout discovered sibling tenants by selecting distinct
    // `recorded_by` values for the workspace. Post-drop, the substrate
    // hosts a single endpoint per daemon — there are no sibling tenants
    // within the same database. Return an empty list.
    let _ = (conn, origin_peer_id, workspace_id);
    Ok(Vec::new())
}
...
pub(crate) fn fanout_stored_shared_event_inline(
    conn: &Connection,
    recorded_by: &str,
    event_id: &EventId,
