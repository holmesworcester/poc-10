...
fn insert_peer_shared_with_user(
    ...
    conn.execute(
        "INSERT INTO peers_shared
             (recorded_by, event_id, workspace_id, public_key, transport_fingerprint, user_event_id)
             VALUES (?1, ?2, ?3, ?4, ?5, ?6)",
        rusqlite::params![
            recorded_by,
            event_id,
...
fn test_malformed_peer_shared_pubkey_skipped() {
    ...
    conn.execute(
        "INSERT INTO peers_shared (recorded_by, event_id, workspace_id, public_key) VALUES (?1, ?2, ?3, ?4)",
        rusqlite::params![recorded_by, "ps_bad", "test-ws", &[0u8; 16][..]],
    )
    .unwrap();
...
fn test_allowed_peers_count_ignores_malformed_rows() {
    ...
    conn.execute(
        "INSERT INTO peers_shared (recorded_by, event_id, workspace_id, public_key) VALUES (?1, ?2, ?3, ?4)",
        rusqlite::params![recorded_by, "ps_short", "test-ws", &[9u8; 16][..]],
    )
    .unwrap();
    conn.execute(
            "INSERT INTO invite_bootstrap_trust
...
fn test_removed_peer_excluded_from_trust() {
    ...
    conn.execute(
        "INSERT INTO peers_shared (recorded_by, event_id, workspace_id, public_key, transport_fingerprint)
             VALUES (?1, ?2, ?3, ?4, ?5)",
        rusqlite::params![
            recorded_by,
            peer_event_id,
