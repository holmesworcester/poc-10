...
/// from invites_accepted projection rows.
...
/// The projection table is now keyed by `(workspace_id, event_id)` and
/// no longer carries a per-peer scope. Callers historically passed a
/// `peer_id` (`recorded_by`) to pick out a single tenant's binding;
/// post-drop, the mapping from peer_id to workspace is no longer
/// derivable from this table alone. The function picks the earliest
/// accepted workspace as a best-effort fall-back so single-tenant
/// scenarios keep working; multi-tenant callers must move to
/// `transport_creds::list_hosted_workspaces` or pass workspace_id
/// directly.
