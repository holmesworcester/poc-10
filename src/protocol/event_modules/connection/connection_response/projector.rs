...
pub fn project(event: &EventWithContext<'_>) -> Result<ProjectionOutput, String> {
    ...
    if let Some(receive) = receive {
        ...
        let initiator = connection_ephemeral::codec::decode(&initiator.canonical_bytes)
        ...
        let responder = connection_ephemeral::codec::decode(&responder.canonical_bytes)
            .map_err(|_| "connection response dependency is not an ephemeral secret".to_string())?;
...
mod tests {
    ...
    fn fixture() -> Fixture {
        ...
        let initiator_ephemeral = connection_ephemeral::types::EphemeralSecretEvent {
            owner_endpoint: [1; 32],
            ephemeral_private_key: [8; 32],
            ephemeral_public_key: crypto::x25519_public_key(&[8; 32]),
            created_at_ms: 0,
        };
        let initiator_ephemeral_record = connection_ephemeral::codec::record_from_bytes(
            connection_ephemeral::codec::encode(&initiator_ephemeral),
        )
        ...
        let responder_ephemeral = connection_ephemeral::types::EphemeralSecretEvent {
            owner_endpoint: responder_endpoint,
            ephemeral_private_key: [9; 32],
            ephemeral_public_key: crypto::x25519_public_key(&[9; 32]),
            created_at_ms: 0,
        };
        let responder_ephemeral_record = connection_ephemeral::codec::record_from_bytes(
            connection_ephemeral::codec::encode(&responder_ephemeral),
        )
