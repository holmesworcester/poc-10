...
//! worker, then writes one admin row. Direct bootstrap grants are authorized by
//! the workspace root. Ongoing grants must be signed by the authority admin key
//! named by the inner grant.
...
fn validate_signed_authority(
    ...
    if admin.authority_event_id == admin.workspace_id {
        return Err("signed admin authority must be a prior admin event".to_string());
    }
