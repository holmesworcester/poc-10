...
//! that the runtime admits through the normal wake loop.
...
use crate::protocol::intent_handlers::bootstrap_send;
...
pub struct Invite {
    ...
    pub user_authority_event_id: Option<FactId>,
...
pub fn create(
    ...
    let (invite_event_id, workspace_id, mut facts) = match input.workspace_id {
        Some(workspace_id) => {
            let authority = local_admin_id(ctx.store(), workspace_id, local.signing_public_key)?;
            let user_invite = identity_user_invite::commands::create_with_secret(
                identity_user_invite::commands::CreateUserInviteWithSecret {
                    created_at_ms: input.created_at_ms.saturating_add(1),
                    workspace_id,
                    authority_event_id: authority.admin_id,
    ...
    let invite_secret = match input.workspace_id {
        Some(workspace_id) => {
            InviteSecretFact::scoped(invite_private_key, workspace_id, invite_event_id)
        }
...
pub fn create_device_link(
    ...
    let user = identity_user::queries::users_in_workspace(ctx.store(), input.workspace_id)?
        .into_iter()
        .find(|user| user.user_id == membership.user_authority_event_id)
        .ok_or_else(|| "local endpoint user is missing".to_string())?;

    let invite_private_key = crypto::random_ed25519_private_key();
    let device_invite = identity_device_invite::fact::DeviceInviteFact {
        created_at_ms: input.created_at_ms.saturating_add(1),
        workspace_id: input.workspace_id,
        user_authority_event_id: user.user_id,
        user_invite_event_id: None,
        public_key: crypto::ed25519_public_key(&invite_private_key),
    };
    let device_invite_bytes =
        crate::protocol::fact_modules::signed_fact::create::sign_payload_bytes(
            membership.endpoint_shared_id,
            &local.signing_secret,
            identity_device_invite::layout::encode_fact(&device_invite)?,
        )?;
    ...
    let link = format_invite(Invite {
        ...
        bootstrap_secret: invite_private_key,
        addr: input.public_addr,
        invite_event_id: device_invite_fact.id,
        workspace_id: input.workspace_id,
        user_authority_event_id: Some(user.user_id),
        endpoint_role: InviteEndpointRole::Device,
        identity_scope: true,
    });
...
pub fn create_invite_server(
    ...
    let invite_private_key = crypto::random_ed25519_private_key();
    let invite_server = identity_invite_server::fact::InviteServerFact {
        created_at_ms: input.created_at_ms.saturating_add(1),
        public_key: crypto::ed25519_public_key(&invite_private_key),
        workspace_id: input.workspace_id,
        authority_event_id: authority.admin_id,
    ...
    let invite_server_bytes =
        crate::protocol::fact_modules::signed_fact::create::sign_payload_bytes(
            authority.signer_id,
            &local.signing_secret,
            identity_invite_server::layout::encode_fact(&invite_server)?,
        )?;
    ...
    let link = format_invite(Invite {
        ...
        bootstrap_secret: invite_private_key,
        addr: input.public_addr,
        invite_event_id: invite_server_fact.id,
        workspace_id: input.workspace_id,
        user_authority_event_id: None,
...
pub fn accept(
    ...
    let request = connection_request::commands::create(
        connection_request::commands::CreateConnectionRequest {
            created_at_ms: input.created_at_ms.saturating_add(1),
            ...
            invite_event_id: input.invite.invite_event_id,
    ...
    if input.invite.identity_scope {
        ...
        let user =
            identity_user::commands::create_signed(identity_user::commands::CreateSignedUser {
                created_at_ms: input.created_at_ms.saturating_add(4),
                workspace_id: input.invite.workspace_id,
                signer_id: input.invite.invite_event_id,
        ...
        let accepted = identity_invite_accepted::commands::accept(
            identity_invite_accepted::commands::AcceptInvite {
    ...
    let send_intent =
        bootstrap_send::bootstrap_send_request_intent(bootstrap_send::BootstrapSendRequest {
...
fn workspace_accept_device_invite_fact(
    ...
    let bytes = crate::protocol::fact_modules::signed_fact::create::sign_payload_bytes(
        user_id,
        &bootstrap_secret,
...
pub fn accept_invite_server(
    ...
    if input.invite.user_authority_event_id.is_some() {
    ...
    let endpoint_shared = endpoint_shared_fact_with_role(
        ...
        local.endpoint,
        local.signing_public_key,
        identity_endpoint_shared::fact::EndpointRole::InviteServer,
        &input.device_name,
        input.invite.invite_event_id,
        input.invite.bootstrap_secret,
    )?;
...
fn endpoint_shared_fact(
    ...
    endpoint_shared_fact_with_role(
        ...
        endpoint_id,
        signing_public_key,
        identity_endpoint_shared::fact::EndpointRole::Device,
        device_name,
        signer_id,
        signer_private_key,
    )
...
fn endpoint_shared_fact_with_role(
    ...
    endpoint_id: FactId,
    signing_public_key: [u8; 32],
    endpoint_role: identity_endpoint_shared::fact::EndpointRole,
    device_name: &str,
    signer_id: FactId,
    signer_private_key: [u8; 32],
) -> Result<Fact, String> {
    ...
    let bytes = crate::protocol::fact_modules::signed_fact::create::sign_payload_bytes(
        signer_id,
        &signer_private_key,
        identity_endpoint_shared::layout::encode_fact(&endpoint)?,
    )?;
    Ok(Fact::new(FactScope::Global, created_at_ms, bytes))
}
...
pub fn parse(value: &str) -> Result<Invite, String> {
    ...
    for part in parts {
        ...
        match label {
            LABEL_INVITE_ID => replace_once(&mut invite_event_id, decode_hex_32(value)?, label)?,
            ...
            LABEL_USER_ID => {
                replace_once(&mut user_authority_event_id, decode_hex_32(value)?, label)?
            }
            LABEL_ENDPOINT_ROLE => {
...
pub fn format_invite(invite: Invite) -> String {
    let mut parts = vec![
        ...
        format!("{LABEL_INVITE_ID}.{}", encode_hex(&invite.invite_event_id)),
        format!(
            "{LABEL_INVITE_PRIVKEY}.{}",
    ...
    if let Some(user_id) = invite.user_authority_event_id {
...
fn local_admin_id(
    ...
    store
        .table_rows_with_key_prefix(identity_admin::rows::ADMIN_ROWS, &workspace_id, usize::MAX)
        .map_err(|err| format!("load admin rows: {err}"))?
        .into_iter()
        .map(|(key, value)| identity_admin::rows::decode_admin_row(&key, &value))
        .collect::<Result<Vec<_>, _>>()?
        .into_iter()
        .find(|admin| admin.user_fact_id == membership.user_authority_event_id)
