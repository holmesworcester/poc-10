...
//! and the dependency edges (invite event, invite secret, initiator ephemeral
//! secret) so downstream consumers can resolve the request without re-decoding
//! the full fact body.

use crate::core::store::{TableName, TableRow};

use super::fact::{ConnectionRequestFact, EndpointId, EventId};
...
pub fn connection_request_key(request_id: &EventId) -> Vec<u8> {
    request_id.to_vec()
}
...
pub fn connection_request_row(
    ...
    value[64..96].copy_from_slice(&fact.invite_event_id);
    value[96..128].copy_from_slice(&fact.invite_secret_event_id);
    value[128..160].copy_from_slice(&fact.initiator_ephemeral_secret_event_id);
    Ok(TableRow {
...
pub fn decode_connection_request_row(
    ...
    if key.len() != 32 {
        return Err("connection request row key must be the request event id".to_string());
    }
    ...
    invite_event_id.copy_from_slice(&value[64..96]);
    let mut invite_secret_event_id = [0; 32];
    invite_secret_event_id.copy_from_slice(&value[96..128]);
    let mut initiator_ephemeral_secret_event_id = [0; 32];
    initiator_ephemeral_secret_event_id.copy_from_slice(&value[128..160]);
