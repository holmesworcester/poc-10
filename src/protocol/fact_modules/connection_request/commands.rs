...
use crate::protocol::fact_modules::identity_invite::{self, fact::InviteSecretFact};
...
pub fn create(
    ...
    let invite_secret = match input.workspace_id {
        Some(workspace_id) => {
            ...
            InviteSecretFact::scoped(input.bootstrap_secret, workspace_id, input.invite_event_id)
        }
    ...
    let invite_secret_fact = Fact::new(
        FactScope::Local,
        input.created_at_ms,
        identity_invite::layout::encode_fact(&invite_secret)?,
    );
    ...
    let mut request = ConnectionRequestFact {
        ...
        invite_secret_event_id: invite_secret_fact.id,
