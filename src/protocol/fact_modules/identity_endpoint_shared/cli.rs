...
use crate::protocol::fact_modules::{identity_endpoint, identity_workspace};
...
pub fn identity(ctx: &CommandContext<'_>, _args: CliArgs<'_>) -> Result<CliOutput, String> {
    ...
    for membership in identity_workspace::local_membership::local_memberships(ctx.store())? {
        lines.push(format!(
            ...
            encode_hex(&membership.endpoint_shared.user_authority_event_id),
    ...
    let lines = queries::peers_in_workspace(ctx.store(), workspace_id)?
        ...
        .map(|peer| {
            format!(
                ...
                encode_hex(&peer.user_authority_event_id),
