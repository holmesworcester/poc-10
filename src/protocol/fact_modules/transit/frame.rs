...
pub fn connection_send_nonce(connection_id: FactId, fact_ids: &[FactId]) -> XChaCha20Poly1305Nonce {
    ...
    hash.update(b"topo:transit-connection-send-nonce:v1:");
...
pub fn open_connection_frame(
    ...
    if parts.ciphertext.len() != expected_ciphertext_len {
        return Err(format!(
            "transit frame ciphertext must fill fixed slot: expected {} got {}",
    ...
    if plaintext.len() != expected_plaintext_len {
        return Err(format!(
            "transit frame plaintext must fill fixed slot: expected {} got {}",
...
fn plaintext_len_for_size_class(size_class: u8) -> Result<usize, String> {
    match size_class {
        ...
        other => Err(format!("unknown transit frame size class {other}")),
    }
}
...
fn inner_bundle_packed_len(facts: &TransitFactBundle) -> Result<usize, String> {
    ...
    for fact in facts.iter() {
        ...
        u32::try_from(fact_len)
            .map_err(|_| format!("transit inner length too large: {fact_len}"))?;
        len = len
...
fn encode_inner_bundle(facts: &TransitFactBundle, plaintext_len: usize) -> Result<Vec<u8>, String> {
    ...
    if packed_len > plaintext_len {
        return Err(format!(
            "transit inner payload too large: max {} got {packed_len}",
            plaintext_len
        ));
    }
...
fn put(out: &mut [u8], offset: &mut usize, bytes: &[u8]) -> Result<(), String> {
    let end = offset
        ...
        .ok_or_else(|| "transit inner bundle length overflow".to_string())?;
    if end > out.len() {
        return Err("transit inner bundle exceeds fixed slot".to_string());
    }
...
impl<'a> Reader<'a> {
    ...
    fn finish_zero_padding(self) -> Result<(), String> {
        if self.bytes[self.offset..].iter().all(|byte| *byte == 0) {
            ...
            Err("transit inner bundle has nonzero padding".to_string())
        }
    }
}
