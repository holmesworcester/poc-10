...
use crate::protocol::fact_modules::content_event::{fact::ContentEventFact, layout, queries};
...
pub fn generate(
    ...
    identity_workspace::queries::workspace_by_id(ctx.store(), workspace_id)?;
...
mod tests {
    ...
    fn generate_constructs_deterministic_content_facts() {
        let store = Store::open_memory_with_schema_sources(&[
            ...
            crate::core::schema_dsl::FACT_MODULES_SCHEMA_SOURCE,
        ])
        .expect("store");
        let clock = FnClock(|| 1);
        let vault = EmptyVault;
        let ctx = CommandContext::new(&store, &clock, &vault);
        let workspace = identity_workspace::commands::create_workspace(&ctx, [7; 32], "workspace")
            .expect("workspace command");
        let workspace_fact = workspace.facts.first().expect("workspace fact");
        let workspace_body =
            identity_workspace::layout::decode_fact(&workspace_fact.bytes).expect("decode");
        store
            .insert_table_rows(vec![identity_workspace::rows::workspace_row(
                workspace_fact.id,
                &workspace_body,
            )
