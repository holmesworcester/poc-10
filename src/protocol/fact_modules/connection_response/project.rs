...
use crate::protocol::fact_modules::connection_ephemeral_secret::layout as ephemeral_layout;
use crate::protocol::fact_modules::connection_request::layout as request_layout;
use crate::protocol::fact_modules::identity_invite::layout as invite_layout;
use crate::protocol::fact_modules::transit_received::{
...
fn validate_response_fields(response: &ConnectionResponseFact) -> Result<(), String> {
    ...
    if response.invite_secret_event_id == [0; 32] {
        return Err("connection response invite_secret_event_id cannot be empty".to_string());
    }
    if response.initiator_ephemeral_secret_event_id == [0; 32] {
        return Err(
            "connection response initiator_ephemeral_secret_event_id cannot be empty".to_string(),
        );
    }
    if response.responder_ephemeral_secret_event_id == [0; 32] {
        return Err(
            "connection response responder_ephemeral_secret_event_id cannot be empty".to_string(),
        );
    }
...
fn validate_request_response(
    ...
    request: &crate::protocol::fact_modules::connection_request::fact::ConnectionRequestFact,
) -> Result<(), String> {
...
fn validate_receive_provenance(
    response_id: [u8; 32],
    response: &ConnectionResponseFact,
    received: &crate::protocol::fact_modules::transit_received::fact::TransitReceivedFact,
) -> Result<(), String> {
...
mod projector_tests {
    ...
    use topo::protocol::fact_modules::connection_ephemeral_secret::{
        fact::ConnectionEphemeralSecretFact, layout as ephemeral_layout,
    };
    use topo::protocol::fact_modules::connection_request::{
        addr::encode_optional_addr, fact::ConnectionRequestFact, layout as request_layout,
    };
    use topo::protocol::fact_modules::connection_response::{create, layout, project, rows};
    ...
    fn invite_match(owner: [u8; 32], invite: Fact) -> MatchedContext {
        MatchedContext {
            need: request_context::connection_invite_secret_need(owner, invite.id),
            offer: request_context::connection_invite_secret_offer(invite.id, invite.id),
            payload: invite,
        }
    }
    ...
    fn receive_match(
        ...
        response_id: [u8; 32],
        request_id: [u8; 32],
        response: &topo::protocol::fact_modules::connection_response::fact::ConnectionResponseFact,
    ) -> MatchedContext {
