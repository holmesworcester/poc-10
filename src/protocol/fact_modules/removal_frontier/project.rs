...
impl Projector for RemovalFrontierProjector {
    fn project(
        ...
        let ref_needs = frontier
            ...
            .copied()
            .map(|ref_id| {
                crate::protocol::matchers::exact_event_need(fact.id, expected_scope.clone(), ref_id)
            })
            .collect::<Vec<_>>();
...
mod projector_tests {
    ...
    use topo::protocol::fact_modules::identity_admin::fact::AdminFact;
    use topo::protocol::fact_modules::identity_admin::layout as admin_layout;

    use topo::protocol::fact_modules::removal_frontier::fact::RemovalFrontierFact;
    use topo::protocol::fact_modules::removal_frontier::{layout, project, rows};
