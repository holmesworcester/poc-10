...
use crate::protocol::fact_modules::content_message::authority::{self, DecodedPayload};
use crate::protocol::fact_modules::content_message::layout as message_layout;
use crate::protocol::fact_modules::identity_user::layout as user_layout;
...
impl Projector for ContentFileProjector {
    fn project(
        ...
        .offer(crate::protocol::matchers::exact_event_offer(
            fact.id,
...
fn validate_file_deletion(
    ...
    let deletion_payload = maybe_signed_payload(
        payload,
        crate::protocol::fact_modules::content_file_deletion::layout::TYPE_CONTENT_FILE_DELETION,
        "file deletion",
    )?;
    let deletion = crate::protocol::fact_modules::content_file_deletion::layout::decode_fact(
...
fn validate_message_deletion(
    ...
    let deletion_payload = maybe_signed_payload(
        payload,
        crate::protocol::fact_modules::content_message_deletion::layout::TYPE_CONTENT_MESSAGE_DELETION,
        "parent deletion",
    )?;
    let deletion = crate::protocol::fact_modules::content_message_deletion::layout::decode_fact(
        &deletion_payload.payload,
    )
...
mod projector_tests {
    ...
    use topo::protocol::fact_modules::content_file::fact::{ContentFileFact, FILE_ROOT_HASH_BYTES};
    use topo::protocol::fact_modules::content_file::{layout, project, rows};
    use topo::protocol::fact_modules::content_message::fact::ContentMessageFact;
    use topo::protocol::fact_modules::content_message::layout as message_layout;
    use topo::protocol::matchers::ExactSelectorMatcher;

    use topo::protocol::fact_modules::identity_user::{fact::UserFact, layout as user_layout};
