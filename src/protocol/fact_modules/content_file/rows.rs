...
//!
//! Rows are keyed by `workspace_id || file_event_id` so callers can scan a
...
pub fn content_file_key(workspace_id: &WorkspaceId, file_event_id: &FactId) -> Vec<u8> {
...
pub fn content_file_row(file_event_id: FactId, fact: &ContentFileFact) -> Result<TableRow, String> {
    ...
    Ok(TableRow {
        table: FILE_ROWS,
        key: content_file_key(&fact.workspace_id, &file_event_id),
        value,
    })
}
...
mod tests {
    ...
    fn content_file_row_round_trips_workspace_keyed_value() {
        ...
        assert_eq!(decoded.file_event_id, [7; 32]);
