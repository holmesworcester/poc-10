...
//!     bits of an `event_id_in_minute`. Internals carry the first `bit_depth`
//!     bits in `event_id_prefix`; leaves sit at `bit_depth = 256` and store
//!     the full event id.
//!
...
/// as `event_id_prefix` and sits at `bit_depth = TRIE_LEAF_BIT_DEPTH`.
