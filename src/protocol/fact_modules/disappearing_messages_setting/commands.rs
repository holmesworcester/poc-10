...
use crate::protocol::fact_modules::{identity_admin, identity_workspace, sealed_message};
...
fn local_admin_user_id(store: &Store, workspace_id: FactId) -> Result<FactId, String> {
    ...
    let user_id = membership.user_authority_event_id;
    ...
    let is_admin = admin_rows.into_iter().any(|(key, value)| {
        identity_admin::rows::decode_admin_row(&key, &value)
