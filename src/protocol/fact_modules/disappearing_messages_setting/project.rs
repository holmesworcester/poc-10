...
impl Projector for DisappearingMessagesSettingProjector {
    fn project(
        ...
        let previous_need = setting.supersedes_setting_id.map(|previous_id| {
            crate::protocol::matchers::exact_event_need(fact.id, FactScope::Global, previous_id)
        });
...
mod projector_tests {
    ...
    use topo::protocol::fact_modules::disappearing_messages_setting::{layout, project, rows};
