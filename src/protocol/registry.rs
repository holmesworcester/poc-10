...
projector_routes! {
    ...
    project_connection_bootstrap_request => connection::bootstrap_request::layout::TYPE_CONNECTION_BOOTSTRAP_REQUEST, connection::bootstrap_request::project::ConnectionBootstrapRequestProjector;
    project_connection_bootstrap_response => connection::bootstrap_response::layout::TYPE_CONNECTION_BOOTSTRAP_RESPONSE, connection::bootstrap_response::project::ConnectionBootstrapResponseProjector;
...
pub(crate) const HANDLER_ROUTES: &[HandlerRoute] = &[
    ...
    // It owns no state: each tick it queries the pending-bootstrap set from the
    // connection request/response read models and queues bootstrap sends.
    handler_route!(
