...
pub fn signing_bytes(fact: &ContentFileSliceFact) -> Result<Vec<u8>, String> {
    wire::canonical_with_zeroed_field(
        &encode_fact(fact)?,
        SIGNATURE_OFFSET..CONTENT_FILE_SLICE_BYTES,
    )
    .map_err(wire_err)
}

pub fn verify_signature(fact: &ContentFileSliceFact) -> Result<(), String> {
