...
fn connection_workspaces(
    ...
    if let Some(invite_secret) = context_change_pipeline::persisted_fact(store, &invite_secret_id)?
    {
...
fn connection_invite_secret_id(
    ...
    if let Some(response_fact) =
        context_change_pipeline::persisted_fact(store, &connection.connection_id)?
    {
...
fn fact_for_shareable_row(store: &Store, row: &ShareableFactRow) -> Result<Option<Fact>, String> {
    let Some(fact) = context_change_pipeline::persisted_fact(store, &row.fact_id)? else {
        return Ok(None);
    };
