...
mod projector_tests {
    ...
    use topo::protocol::facts::identity::endpoint_shared::{layout, project, rows};
    use topo::protocol::facts::identity::invite_server::{
    ...
    fn endpoint_shared_projector_materializes_row_with_invite_server_context() {
        ...
        assert_eq!(row.user_authority_fact_id, [2; 32]);
    ...
    fn invite_server_endpoint_shared() -> (EndpointSharedFact, Fact, Fact) {
        ...
        let payload = EndpointSharedFact {
            user_authority_fact_id: invite_server_fact.id,
            ..sample_fact()
        };
        let fact =
    ...
    fn signed_invite_server_fact(
        ...
        Fact {
            ...
            timestamp: 1,
            bytes: identity::signed_fact::create::sign_payload_bytes(
                workspace_id,
                &INVITE_SERVER_PRIVATE_KEY,
    ...
    fn signed_device_invite_fact(
        ...
        let payload = topo::protocol::facts::identity::device_invite::layout::encode_fact(
            &topo::protocol::facts::identity::device_invite::fact::DeviceInviteFact {
                created_at_ms: 1,
                workspace_id,
                user_authority_fact_id,
                user_invite_fact_id: Some([6; 32]),
