...
/// the `identity::signed_fact` envelope around the inner sealed-message payload. The
...
/// `identity::signed_fact::layout::decode_signed_fact` first.
///
...
pub fn send_message(
    ...
    // emitted fact carries authentication. The identity::signed_fact module owns the
    ...
    // sealed-message bytes call `identity::signed_fact::layout::decode_signed_fact` on
    // `fact.bytes` and then `content::sealed_message::layout::decode_sealed_message`
