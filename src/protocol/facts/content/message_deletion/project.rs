...
mod projector_tests {
    ...
    fn content_message_deletion_projector_materializes_authorized_author_delete() {
        ...
        assert_eq!(
            stored.values[0],
            topo::core::intents::SqlValue::Bytes(deletion.workspace_id.to_vec())
        );
        assert_eq!(
            stored.values[1],
            topo::core::intents::SqlValue::Bytes(deletion.target_message_id.to_vec())
        );
        assert_eq!(
            stored.values[2],
            topo::core::intents::SqlValue::Bytes(fact.id.to_vec())
        );
        assert_eq!(stored.values[3], topo::core::intents::SqlValue::U64(12_345));
        assert_eq!(
            stored.values[4],
            topo::core::intents::SqlValue::Bytes(deletion.author_user_id.to_vec())
        );
    }
