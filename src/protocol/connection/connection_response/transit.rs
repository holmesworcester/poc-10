//! Wire transport for membership connection-response facts.
//!
//! A membership connection response is sent on the wire before established
//! frames exist, so its canonical fact bytes are sealed for transit
//! responder-ephemeral → initiator-endpoint and opened back to the identical
//! canonical bytes, admitted as the durable `connection_response` fact. The
//! seal header is transport-only and never part of the fact.
//!
//! Change this file for membership-response wire-transport compatibility. The
//! durable fact layout lives in `layout.rs`; handshake derivation in
//! `create.rs`; admission in `project.rs`.
