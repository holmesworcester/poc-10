...
pub const TYPE_BOOTSTRAP_RESPONSE_RECEIVED: u8 = 182;
pub const FACT_BYTES: usize = 1 + 32 + 32 + 32 + 8;

pub fn encode_fact(fact: &BootstrapResponseReceivedFact) -> Result<Vec<u8>, String> {
    let mut out = vec![0; FACT_BYTES];
    wire::put_u8(TYPE_BOOTSTRAP_RESPONSE_RECEIVED, &mut out[0..1]).map_err(wire_err)?;
...
pub fn decode_fact(bytes: &[u8]) -> Result<BootstrapResponseReceivedFact, String> {
    wire::expect_len(bytes, FACT_BYTES).map_err(wire_err)?;
    if wire::take_u8(&bytes[0..1]).map_err(wire_err)? != TYPE_BOOTSTRAP_RESPONSE_RECEIVED {
        return Err("expected bootstrap_response_received fact".to_string());
    }
    ...
    Ok(BootstrapResponseReceivedFact {
        response_id,
        request_id,
        receive_id,
...
mod tests {
    ...
    fn fact() -> BootstrapResponseReceivedFact {
    ...
    #[test]
    fn bootstrap_response_received_roundtrip_fixed_width() {
