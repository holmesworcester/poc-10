...
impl IntentHandler for SendBootstrapConnectionRequestHandler {
    ...
    fn handle(&self, intent: &Intent, context: &HandlerContext) -> HandlerResult {
        ...
        let sealed = crate::protocol::connection::bootstrap::seal_connection_request(
            request_fact.body(),
...
mod tests {
    ...
    fn sends_sealed_bootstrap_request_bytes() {
        ...
        assert_eq!(sent[0], bootstrap::TYPE_SEALED_CONNECTION_REQUEST);
        ...
        assert_eq!(
            bootstrap::open_connection_request(&sent, &responder).expect("open request"),
            request_fact.bytes
        );
    }
