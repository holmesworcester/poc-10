//! Membership connection-response network-send intent.
//!
//! The responder creates its ephemeral and `connection_response` facts and the
...
//! bounded write to the initiator's return address.
//!
//! The intent key is the response id, so duplicate sends collapse. A lost send
//! is recovered by a later live retry from the committed response.
...
pub struct SendConnectionResponse {
    pub response_id: [u8; 32],
    pub responder_ephemeral_secret_id: [u8; 32],
    pub addr: SocketAddr,
}

pub fn send_connection_response_intent(input: SendConnectionResponse) -> Result<Intent, String> {
    ...
    Ok(Intent::new(
        IntentKind::new(SEND_CONNECTION_RESPONSE)
            .expect("valid send connection response intent kind"),
        send_connection_response_key(&input),
        payload,
    ))
}

pub fn decode_send_connection_response(intent: &Intent) -> Result<SendConnectionResponse, String> {
    if intent.kind.as_str() != SEND_CONNECTION_RESPONSE {
        return Err("expected send_connection_response intent".into());
    }
    if intent.payload.len() != 1 + 32 + 32 + addr::ADDR_BLOCK_BYTES {
        return Err("send_connection_response payload has wrong length".into());
    }
    if intent.payload[0] != 1 {
        return Err("send_connection_response payload version unsupported".into());
    }
    ...
    let addr = addr::decode_optional_addr(&addr_bytes)?
        .ok_or_else(|| "send_connection_response addr is missing".to_string())?;
    let input = SendConnectionResponse {
        response_id,
        responder_ephemeral_secret_id,
        addr,
    };
    if intent.key != send_connection_response_key(&input) {
        return Err("send_connection_response key does not match payload".into());
    }
    Ok(input)
}

#[derive(Debug, Clone, Default)]
pub struct SendConnectionResponseHandler;

impl SendConnectionResponseHandler {
    pub fn new() -> Self {
        Self
    }
}

impl IntentHandler for SendConnectionResponseHandler {
    fn input_fact_ids(&self, intent: &Intent) -> Result<Vec<HandlerFactId>, String> {
        let input = decode_send_connection_response(intent)?;
    ...
    fn handle(&self, intent: &Intent, context: &HandlerContext) -> HandlerResult {
        ...
        if response.responder_ephemeral_secret_fact_id != input.responder_ephemeral_secret_id {
            return Err("send_connection_response ephemeral id does not match response".into());
        }
...
fn send_connection_response_key(input: &SendConnectionResponse) -> Vec<u8> {
    let mut hash = blake3::Hasher::new();
    hash.update(b"topo:send-membership-connection-response:v1:");
