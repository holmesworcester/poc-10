...
pub const TYPE_CONNECTION_REQUEST_RECEIVED: u8 = 175;
pub const FACT_BYTES: usize = 1 + 32 + 32 + 8;

pub fn encode_fact(fact: &ConnectionRequestReceivedFact) -> Result<Vec<u8>, String> {
    let mut out = vec![0; FACT_BYTES];
    wire::put_u8(TYPE_CONNECTION_REQUEST_RECEIVED, &mut out[0..1]).map_err(wire_err)?;
...
pub fn decode_fact(bytes: &[u8]) -> Result<ConnectionRequestReceivedFact, String> {
    wire::expect_len(bytes, FACT_BYTES).map_err(wire_err)?;
    if wire::take_u8(&bytes[0..1]).map_err(wire_err)? != TYPE_CONNECTION_REQUEST_RECEIVED {
        return Err("expected connection_request_received fact".to_string());
    }
    ...
    Ok(ConnectionRequestReceivedFact {
        request_id,
        receive_id,
...
mod tests {
    ...
    fn fact() -> ConnectionRequestReceivedFact {
        ConnectionRequestReceivedFact {
            request_id: [1; 32],
            receive_id: [2; 32],
    ...
    #[test]
    fn connection_request_received_roundtrip_fixed_width() {
