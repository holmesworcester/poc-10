...
fn project_receiver_request(
    ...
    let output = output.offer(connection_request_offer(fact.id, fact.id));
    // The connection request is a durable retained fact. Its receipt (which bakes
    // in the origin address) and the create_connection intent it seeds are both
    // derived from volatile incoming metadata, so they belong to the incoming run
    // only. On replay the metadata is gone: the durable receipt fact reprojects on
    // its own and the connection it seeded is already retained, so the request
    // re-publishes its offer without depending on receive metadata here.
    let Some(metadata) = context.incoming_metadata() else {
        if context.is_replay() {
            return Ok(output);
        }
        return Err("connection request missing incoming origin metadata".to_string());
    };
