    ...
    mod tests {
        ...
        fn canonical_fact() -> Fact {
            ...
            Fact::new(
                FactScope::Local,
                100,
                frame_encode::encode_fact(&ConnectionFrameFileSliceFact {
                    frame: FixedSlot::new(&frame).expect("frame slot"),
                })
                .expect("connection_frame_file_slice fact"),
            )
        }
...
pub fn open_connection_frame(
    ...
    if parts.ciphertext.len() != super::encode::CONNECTION_FRAME_FILE_SLICE_CIPHERTEXT_BYTES {
        return Err(format!(
            ...
            super::encode::CONNECTION_FRAME_FILE_SLICE_CIPHERTEXT_BYTES,
    ...
    if plaintext.len() != super::encode::CONNECTION_FRAME_FILE_SLICE_PLAINTEXT_BYTES {
        return Err(format!(
            "connection::frame frame plaintext must fill fixed slot: expected {} got {}",
            super::encode::CONNECTION_FRAME_FILE_SLICE_PLAINTEXT_BYTES,
            plaintext.len()
        ));
    }
