...
//! and the wire encoding of one file-slice encrypted connection frame. It does not
...
/// Public tag prefix for file_slice connection frames.
...
/// Size class byte for file_slice frames.
pub const CONNECTION_FRAME_SIZE_CLASS_FILE_SLICE: u8 = 1;

/// Plaintext capacity of a file_slice connection frame.
...
/// Ciphertext slot capacity for file_slice frames (plaintext + AEAD tag).
...
/// Outer wire length for a file_slice connection frame.
...
pub const CONNECTION_FRAME_FILE_SLICE_FACT_BYTES: usize =
    1 + wire::FixedSlot::<CONNECTION_FRAME_FILE_SLICE_WIRE_BYTES>::LEN;
...
/// Fixed-width file_slice connection frame.
...
pub fn encode_fact(fact: &ConnectionFrameFileSliceFact) -> Result<Vec<u8>, String> {
    if fact.frame.len() != CONNECTION_FRAME_FILE_SLICE_WIRE_BYTES {
        return Err(format!(
            "connection frame file_slice bytes must be {} bytes",
            CONNECTION_FRAME_FILE_SLICE_WIRE_BYTES
        ));
    }
    ConnectionFrameFileSliceV1::decode(fact.frame.bytes()).map_err(wire_err)?;
    let mut out = wire::Writer::with_capacity(CONNECTION_FRAME_FILE_SLICE_FACT_BYTES);
    out.u8(TYPE_CONNECTION_FRAME_FILE_SLICE);
    out.fixed_slot_value(&fact.frame).map_err(wire_err)?;
    out.finish_exact(CONNECTION_FRAME_FILE_SLICE_FACT_BYTES)
        .map_err(wire_err)
}

/// Encode a file_slice connection frame from already-encrypted bytes.
pub fn encode_frame_bytes(
    connection_id: Id32,
    nonce: Nonce24,
    ciphertext: &[u8],
) -> Result<Vec<u8>, WireError> {
    if ciphertext.len() > CONNECTION_FRAME_FILE_SLICE_CIPHERTEXT_BYTES {
        return Err(WireError::ValueTooLarge {
            max: CONNECTION_FRAME_FILE_SLICE_CIPHERTEXT_BYTES,
            actual: ciphertext.len(),
        });
    }
    let mut out = vec![0; CONNECTION_FRAME_FILE_SLICE_WIRE_BYTES];
    encode_header(
        &mut out,
        CONNECTION_FRAME_FILE_SLICE_WIRE_BYTES,
        &connection_id,
...
pub(crate) fn frame_associated_data(
    ...
    out.push(CONNECTION_FRAME_SIZE_CLASS_FILE_SLICE);
