...
use super::rows::{
    decode_observed_endpoint_address_row, observed_endpoint_address_key,
    CONNECTION_OBSERVED_ENDPOINT_ADDRESS_ROWS,
};

/// The last reachable listen address learned for `endpoint_id`, if any.
pub fn observed_endpoint_address(
    store: &Store,
    endpoint_id: &FactId,
    ...
    let Some(value) = store
        .table_row(
            CONNECTION_OBSERVED_ENDPOINT_ADDRESS_ROWS,
            &observed_endpoint_address_key(endpoint_id),
        )
    ...
    let (_, address) =
        decode_observed_endpoint_address_row(&observed_endpoint_address_key(endpoint_id), &value)?;
    Ok(Some(address))
}
