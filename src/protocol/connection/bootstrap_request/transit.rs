//! Wire transport for connection-request facts.
//!
//! A connection request is what we send on the wire before any connection
//! secret exists, so its canonical fact bytes are sealed for transit rather than
//! wrapped in a separate envelope fact. Sealing encrypts the request bytes
//! initiator-ephemeral → responder-endpoint; opening recovers the identical
//! canonical bytes, which are then admitted as the durable `connection_request`
//! fact. The seal header (type tag, version, initiator ephemeral public key,
//! nonce) is transport-only and never part of the fact.
//!
//! Change this file for request wire-transport compatibility. The durable fact
//! layout lives in `layout.rs`; signature transcript and address conversion live
//! in `create.rs`; admission lives in `project.rs`.
...
/// First wire byte of a sealed connection-request frame.
...
/// Whether `frame` is a sealed connection-request frame (by its first byte).
...
/// Open a sealed connection-request frame back to canonical request bytes.
