...
pub const TYPE_BOOTSTRAP_REQUEST: u8 = 42;
...
pub fn encode_fact(fact: &BootstrapRequestFact) -> Result<Vec<u8>, String> {
    let mut out = vec![0; FACT_BYTES];
    wire::put_u8(TYPE_BOOTSTRAP_REQUEST, &mut out[0..1]).map_err(wire_err)?;
...
pub fn decode_fact(bytes: &[u8]) -> Result<BootstrapRequestFact, String> {
    wire::expect_len(bytes, FACT_BYTES).map_err(wire_err)?;
    let tag = wire::take_u8(&bytes[0..1]).map_err(wire_err)?;
    if tag != TYPE_BOOTSTRAP_REQUEST {
...
mod tests {
    ...
    fn rejects_wrong_tag_or_length() {
        ...
        bytes[0] = TYPE_BOOTSTRAP_REQUEST.wrapping_add(1);
