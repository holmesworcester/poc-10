pub mod decode {
    ...
    use super::super::encode::{
        wire_err, ConnectionFrameSmallV1, CIPHERTEXT_OFFSET, CONNECTION_FRAME_HEADER_BYTES,
        CONNECTION_FRAME_SIZE_CLASS_SMALL, CONNECTION_FRAME_SMALL_CIPHERTEXT_BYTES,
        CONNECTION_FRAME_SMALL_FACT_BYTES, CONNECTION_FRAME_SMALL_WIRE_BYTES, CONNECTION_FRAME_TAG,
        CONNECTION_FRAME_VERSION, TYPE_CONNECTION_FRAME_SMALL,
    };
    use super::super::fact::ConnectionFrameSmallFact;
    ...
    /// Borrowed small frame payload recovered without materializing a large fixed
    ...
    pub fn decode_fact(bytes: &[u8]) -> Result<ConnectionFrameSmallFact, String> {
        ...
        reader
            .expect_len(CONNECTION_FRAME_SMALL_FACT_BYTES)
            .map_err(wire_err)?;
        reader
            .expect_u8(TYPE_CONNECTION_FRAME_SMALL)
            .map_err(wire_err)?;
        let frame = reader
            .fixed_slot_value::<CONNECTION_FRAME_SMALL_WIRE_BYTES>()
            .map_err(wire_err)?;
        reader.finish().map_err(wire_err)?;
        ConnectionFrameSmallV1::decode(frame.bytes()).map_err(wire_err)?;
    ...
    /// Inspect the public header of a small connection frame.
    pub fn peek_frame_header(bytes: &[u8]) -> Result<ConnectionFrameHeader, WireError> {
        ...
        if size_class != CONNECTION_FRAME_SIZE_CLASS_SMALL {
    ...
    /// Decode the public header and borrowed ciphertext slot for a small frame.
    pub fn decode_frame_parts(bytes: &[u8]) -> Result<ConnectionFrameParts<'_>, WireError> {
        let header = peek_frame_header(bytes)?;
        if bytes.len() != CONNECTION_FRAME_SMALL_WIRE_BYTES {
            return Err(WireError::WrongLength {
                expected: CONNECTION_FRAME_SMALL_WIRE_BYTES,
                actual: bytes.len(),
            });
        }
        let slot = &bytes[CIPHERTEXT_OFFSET..];
        let ciphertext_len = wire::take_u32be(&slot[..wire::U32_BYTES])? as usize;
        if ciphertext_len > CONNECTION_FRAME_SMALL_CIPHERTEXT_BYTES {
            return Err(WireError::ValueTooLarge {
                max: CONNECTION_FRAME_SMALL_CIPHERTEXT_BYTES,
    ...
    mod tests {
        ...
        use crate::protocol::connection::frame_small::encode::{
            encode_fact, encode_frame_bytes, CONNECTION_FRAME_SMALL_FACT_BYTES,
        };
...
pub mod adapt {
    ...
    pub(crate) fn adapt(
        source: ConnectionFrameSmallFact,
    ) -> Result<ConnectionFrameSmallFact, String> {
        Ok(source)
    }
}
...
pub fn open_connection_frame(
    ...
    if parts.ciphertext.len() != super::encode::CONNECTION_FRAME_SMALL_CIPHERTEXT_BYTES {
        return Err(format!(
            "connection::frame frame ciphertext must fill fixed slot: expected {} got {}",
            super::encode::CONNECTION_FRAME_SMALL_CIPHERTEXT_BYTES,
    ...
    if plaintext.len() != super::encode::CONNECTION_FRAME_SMALL_PLAINTEXT_BYTES {
        return Err(format!(
            "connection::frame frame plaintext must fill fixed slot: expected {} got {}",
            super::encode::CONNECTION_FRAME_SMALL_PLAINTEXT_BYTES,
            plaintext.len()
        ));
    }
    let inner = decode_packed_inner_bundle(&plaintext)?;
