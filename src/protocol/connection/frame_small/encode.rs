...
/// Public tag prefix for small connection frames.
...
/// Size class byte for small frames.
pub const CONNECTION_FRAME_SIZE_CLASS_SMALL: u8 = 0;

/// Plaintext capacity of a small connection frame.
...
/// Outer wire length for a small connection frame.
...
pub const CONNECTION_FRAME_SMALL_FACT_BYTES: usize =
    1 + wire::FixedSlot::<CONNECTION_FRAME_SMALL_WIRE_BYTES>::LEN;

const FRAME_PURPOSE: &[u8] = b"topo connection::frame frame v1";

/// Fixed-width small connection frame.
...
pub fn encode_fact(fact: &ConnectionFrameSmallFact) -> Result<Vec<u8>, String> {
    if fact.frame.len() != CONNECTION_FRAME_SMALL_WIRE_BYTES {
        return Err(format!(
            "connection frame small bytes must be {} bytes",
            CONNECTION_FRAME_SMALL_WIRE_BYTES
        ));
    }
    ConnectionFrameSmallV1::decode(fact.frame.bytes()).map_err(wire_err)?;
    let mut out = wire::Writer::with_capacity(CONNECTION_FRAME_SMALL_FACT_BYTES);
    out.u8(TYPE_CONNECTION_FRAME_SMALL);
    out.fixed_slot_value(&fact.frame).map_err(wire_err)?;
    out.finish_exact(CONNECTION_FRAME_SMALL_FACT_BYTES)
        .map_err(wire_err)
}

/// Encode a small connection frame from already-encrypted bytes.
pub fn encode_frame_bytes(
    connection_id: Id32,
    nonce: Nonce24,
    ciphertext: &[u8],
) -> Result<Vec<u8>, WireError> {
    if ciphertext.len() > CONNECTION_FRAME_SMALL_CIPHERTEXT_BYTES {
        return Err(WireError::ValueTooLarge {
            max: CONNECTION_FRAME_SMALL_CIPHERTEXT_BYTES,
            actual: ciphertext.len(),
        });
    }
    let mut out = vec![0; CONNECTION_FRAME_SMALL_WIRE_BYTES];
    encode_header(
        &mut out,
        CONNECTION_FRAME_SMALL_WIRE_BYTES,
        &connection_id,
...
pub(crate) fn frame_associated_data(
    ...
    out.push(CONNECTION_FRAME_SIZE_CLASS_SMALL);
