...
//! fact_id_prefix)` coordinate so the projector can address time-tree
//! internals (`bit_depth = 0, fact_id_prefix = 0`), minute_nodes
//! (`bit_depth = 0, range_width = 1`), trie internals (`1..=255`) and trie
//! leaves (`bit_depth = TRIE_LEAF_BIT_DEPTH`) without changing wire shape.
//!
//! The encoded `fact_id_prefix` is always masked to `bit_depth` so equal
...
pub fn encode_fact(fact: &LocalHistoryNodeSecretFact) -> Result<Vec<u8>, String> {
    ...
    out[115..147].copy_from_slice(&mask_prefix_to_depth(fact.fact_id_prefix, fact.bit_depth));
...
pub fn decode_fact(bytes: &[u8]) -> Result<LocalHistoryNodeSecretFact, String> {
    ...
    let fact = LocalHistoryNodeSecretFact {
        ...
        fact_id_prefix: bytes[115..147].try_into().unwrap(),
        tombstone_node_id: bytes[147..179].try_into().unwrap(),
        node_secret: bytes[179..179 + NODE_SECRET_BYTES].try_into().unwrap(),
    };
    validate(&fact)?;
    if mask_prefix_to_depth(fact.fact_id_prefix, fact.bit_depth) != fact.fact_id_prefix {
        return Err(
            "local history node secret fact_id_prefix is not masked to bit_depth".to_string(),
        );
    }
    Ok(fact)
}
