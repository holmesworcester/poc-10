...
use crate::protocol::encryption::fact::{LocalHistoryNodeSecretFact, LocalKeySecretFact};
use crate::protocol::encryption::layout;
use crate::protocol::encryption::local_history_node_secret::fact::{
...
pub(super) fn project_local_key_secret(
    ...
    let scope = crate::protocol::identity::workspace::scope(secret.workspace_id);
    ...
    for offer in wrap_source::frontier_root_wrap_source_offers(
        fact.id,
        scope.clone(),
        secret.workspace_id,
        secret.frontier_id,
...
pub(super) fn project_local_history_node_secret(
    ...
    let scope = crate::protocol::identity::workspace::scope(node.workspace_id);
    require_local_scope(fact)?;
    let frontier_need = crate::core::context::ContextNeed::range(
        fact.id,
        "encryption_removal_frontier",
    ...
    for offer in wrap_source::history_node_wrap_source_offers(
        fact.id,
        scope.clone(),
        node.workspace_id,
        node.frontier_id,
