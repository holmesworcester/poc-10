//! Key wrap projector plus shared encryption wrap-source policy.
//!
...
// ---------------------------------------------------------------------------
// Shared projection helpers consumed by the other encryption families.
// ---------------------------------------------------------------------------
...
pub(crate) fn has_matching_signer_public_key(
    ...
    projection_context
        ...
        .any(|(_, payload)| {
            ...
            let Ok(endpoint) = identity::endpoint_shared::decode_fact_payload(&envelope.payload)
            else {
                return false;
            };
...
fn signed_key_wrap(
    ...
    if let Some(local_recipient_fact) = local_recipient_fact {
        ...
        let local = crate::protocol::encryption::local_recipient_key::decode_fact_payload(
...
mod wrap_source_tests {
    use super::*;
    use crate::protocol::identity::workspace::scope;
