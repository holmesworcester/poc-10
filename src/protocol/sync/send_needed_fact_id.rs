...
use crate::protocol::sync::{have_id, need_id, shared_fact};
...
pub fn decode_send_needed_fact_id(intent: &Intent) -> Result<SendNeededFactId, String> {
    ...
    if intent.handler_key != send_needed_fact_id_key(&input) {
        return Err("send_needed_fact_id intent key does not match payload".into());
    }
    Ok(input)
}
