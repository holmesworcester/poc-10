//! Driver for retention purge of accepted facts.

use crate::core::handler_dispatch::{HandlerContext, HandlerFactId, HandlerOutput, IntentHandler};
use crate::core::intents::Intent;
use crate::event_modules::sealed_message::{intent, layout};
...
impl IntentHandler for PurgeEventHandler {
    ...
    fn input_fact_ids(&self, raw_intent: &Intent) -> Result<Vec<HandlerFactId>, String> {
        let input = intent::decode_purge_event_intent(raw_intent)?;
    ...
    fn handle(
        ...
        if input.target_kind != intent::PURGE_TARGET_MESSAGE {
            return Err("purge_event target kind is not supported".to_string());
        }
        if input.reason_kind != intent::PURGE_REASON_AUTHOR_DELETION {
            return Err("purge_event reason kind is not supported".to_string());
        }
