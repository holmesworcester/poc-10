...
- `bootstrap_connect`: sends invite bootstrap requests from the accepting side.
  Unscoped invites carry connection requests. Identity invites carry one
  invite-key batch of the acceptor's shared identity bootstrap events. The
  worker records the invite address as the first normal route; it does not own
  daemon socket accept and it never pulls invite-owner ancestry back on the
  accept stream.
